...
def setup_parser():
    ...
    '''
    parser = argparse.ArgumentParser(prog='brahma config', description='\033[1mbrahma configure [args]\033[0m\n Configure the workspace.',
                                     formatter_class=argparse.RawTextHelpFormatter)

    parser.add_argument(
        '--upstream-deps', help='Adds all upstream dependencies of the packages to the workspace.', action='store_true')
    parser.add_argument(
        '--no-upstream-deps', help='Do not add upstream dependencies of the packages to the workspace.', action='store_true')
    parser.add_argument(
        '--downstream-deps', help='Adds all downstream dependencies of the packages to the workspace.', action='store_true')
    parser.add_argument('--no-downstream-deps',
                        help='Do not add downstream dependencies of the packages to the workspace.', action='store_true')
    parser.add_argument(
    ...
    parser.add_argument('--no-missing-deps',
                        help='Do not add all dependencies that are not installed to the workspace.', action='store_true')

    parser.add_argument("--add-filter-packages", nargs="+",
                        help='Add package to the list of packages that are used to filter the package tree.', type=str, default=list())
    parser.add_argument("--remove-filter-packages", nargs="+",
                        help='Remove package from the list of packages that are used to filter the package tree.', type=str, default=list())
    ...
    parser.add_argument("--add-explicit-packages", nargs="+",
                        help='Add packages to the list of packages explicitly added to the workspace.', type=str, default=list())
    parser.add_argument("--remove-explicit-packages", nargs="+",
                        help='Remove packages from the list of packages explicitly added to the workspace.', type=str, default=list())
