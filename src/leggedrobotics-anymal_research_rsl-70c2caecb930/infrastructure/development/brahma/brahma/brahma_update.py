    ...
    if settings.complete_overlay:
                ...
                if args.pull:
                    ...
                    if git_repo.head.is_detached:
                        ...
                        if current_branch != repo_base_branch and args.pull_and_merge:
                            log.status("Merge")
                            log.info("Merge base branch '{}' into current branch '{}'.".format(repo_base_branch, current_branch))
        ...
        if settings.include_downstream_dependencies and diff_packages:
            ...
            if settings.filter_packages and settings.only_filter_downstream_packages and downstream_packages:
                ...
                downstream_packages = utils.getFilteredPackages(downstream_packages, filter_packages, filter_packages_tree)
    ...
    if catkin_packages:
        ...
        if missing_packages:
            ...
            if settings.include_missing_dependencies:
                ...
                log.warn('Configuration for "include_missing_dependencies" is set to "false". Will not add missing dependencies.')
...
def setup_parser():
    ...
    parser.add_argument(
        "--pull-and-merge", help='Pull changes from the remote for the current branch and the base branch. Merge base branch into the current branch.', action='store_true')
    return parser
