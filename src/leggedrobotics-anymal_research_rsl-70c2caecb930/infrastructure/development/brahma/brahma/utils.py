...
try:
    ...
    log.error("The current directory does not exist in the base branch. Try to run the command from the root of your brahma workspace.")
...
def updateAndCleanPackages(packages, catkin_ws, packages_dir):
    ...
    '''
    # Calculate different package sets
    ...
    built_packages = catkin_ws.getBuiltPackages()
    for removed_package in removed_packages:
        if removed_package in built_packages.keys():
    ...
    if changed_packages:
        ...
        log.info('Resolved {} downstream packages of newly added or removed packages'.format(len(existing_changed_packages_downstream_deps)))
        log.packages(existing_changed_packages_downstream_deps)

    # Only clean existing packages
    packages_to_clean = set(packages_to_clean & set(built_packages.keys()))
...
def warnIfDeletedHeaders(repo, base_branch):
    ...
    if headers:
        log.warn('Found deleted header files. Overlaying could lead to unexpected behavior, the headers files are still present in debians.')
        log.info('{}'.format("\n".join(headers)))
