...
def setupCLionProject(git_dir, ide_dir, project, packages, catkin_ws):
    ...
    # Copy run configuration settings
    run_configuration_files = os.listdir(run_configs_internal_path)
    for run_configuration_file in run_configuration_files:
        full_file_name = os.path.join(run_configs_internal_path, run_configuration_file)
        shutil.copy(full_file_name, run_configurations_path)
    ...
    subs = {"project": project, "packages": package_paths, "catkin_devel_or_install_space": catkin_ws.develOrInstallSpace(),
            "ros_path": catkin_ws.extendPath()}
    utils.updateFileFromTemplate(os.path.join(script_path, "CMakeLists.txt.em"), os.path.join(clion_path, "CMakeLists.txt"), subs)

    # Set project name
    utils.updateFileFromTemplate(os.path.join(script_path, "name.em"), os.path.join(project_settings_path, ".name"), {"project": project})
