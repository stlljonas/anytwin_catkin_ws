...
class OptimizationNelderMead {
 public:
  using Vector = Eigen::Matrix<double, Vars, 1>;

 private:
  using Fcn = std::function<double(const Vector&)>;
 ...
 public:
  OptimizationNelderMead() = default;
  virtual ~OptimizationNelderMead() = default;

  bool optimize(double& fOpt, Vector& xOpt, const Vector& x0, const Fcn& fcn, const bool minimize = true, const long int maxIterations = -1,
                const double maxFValueDiff = 1e-4, const double maxVertexDiff = 1e-4) {
 ...
 private:
  int getIterations() { return currentIteration_; }
  ...
  void setupSimplex() {
    ...
    for (long int k = 0; k < numCoordinates_; k++) {
      Vector y = x0_;
      if (y[k] != 0) {
        y[k] = (1 + usual_d) * y[k];
      } else {
  ...
  void runMinimizationStep() {
    ...
    Vector c = Vector::Zero(numCoordinates_);
    for (long int k = 0; k < h_; k++) {
      c += simplex_.col(k);
    }
    c /= h_;

    // Compute the reflection point.
    const Vector xr = (1.0 + rho_) * c - rho_ * simplex_.col(h_);
    const double fr = evalFcn(xr);

    if (fr < fValues_[0]) {
      // Compute the expansion point.
      const Vector xe = (1.0 + rho_ * chi_) * c - rho_ * chi_ * simplex_.col(h_);
      ...
      if (fr < fValues_[s_]) {
        ...
        // Contraction.
        if (fr < fValues_[h_]) {
          const Vector xc = (1.0 + psi_ * rho_) * c - psi_ * rho_ * simplex_.col(h_);
          ...
          const Vector xcc = (1.0 - psi_) * c + psi_ * simplex_.col(h_);
  ...
  bool terminateOptimization() {
    ...
    for (Eigen::Index k = 0; k < simplexFirstColCopy.cols(); k++) {
    ...
    return maxFValueDiff <= maxFValueDiff_ && maxVertexDiff <= maxVertexDiff_;
  }
  ...
  static std::vector<int> getSortedIndexes(const std::vector<T>& v) {
    ...
    auto comparator = [&v](int i1, int i2) -> bool { return (v[i1] < v[i2]); };
  ...
  void shrinkSimplex() {
    for (long int k = 1; k < numVertices_; k++) {
