...
GravityCompensation::GravityCompensation(const AnydrivePtr& anydrive, const double motorVelocityAbs, const uint64_t turns)
...
bool GravityCompensation::collectData() {
  if (stopRequested()) {
    return false;
  }
  ...
  if (readFromFiles) {
    ...
    for (unsigned int i = 0; i < n_; i++) {
      ...
      ANYDRIVE_INFO("Turning with motor v = " << motorVelocity << " rad/s for " << turns_ << " turns (" << names_[i]
                                              << " measurement series).");
      stageCommandMotorVelocity(motorVelocity);
...
bool GravityCompensation::postProcessData() {
  ANYDRIVE_INFO("Gravity Compensation: Fitting function.");
...
void GravityCompensation::fitFunction(Vector& params) const {
  Vector params0;
  getInitializationParams(params0);
  ANYDRIVE_INFO("Gravity compensation: Initialization parameters:");
  ANYDRIVE_INFO(paramsToString(params0));

  Optimizer optimizer;
  double error = 0;
  optimizer.optimize(error, params, params0, std::bind(&GravityCompensation::getError, this, std::placeholders::_1), true);

  ANYDRIVE_INFO("Gravity compensation: Optimized parameters before check:");
  ANYDRIVE_INFO(paramsToString(params));

  makeParamsUnique(params);

  ANYDRIVE_INFO("Gravity compensation: Optimized parameters:");
  ANYDRIVE_INFO(paramsToString(params));
  ANYDRIVE_INFO("Gravity compensation: Optimization residual error: " << error);
}

void GravityCompensation::getInitializationParams(Vector& params0) const {
...
void GravityCompensation::makeParamsUnique(Vector& params) {
  if (params[1] < 0) {
    params[1] *= -1.0;
    params[2] += M_PI;
  }
...
std::string GravityCompensation::paramsToString(const Vector& params) {
  std::stringstream ss;
  ss << params[0] << "," << params[1] << "," << params[2];
  return ss.str();
}

double GravityCompensation::getGravityCompensation(const Vector& params, const double jointPositionTicks) const {
  return params[0] + params[1] * std::sin(static_cast<double>(omega_ * jointPositionTicks + params[2]));
}

double GravityCompensation::getError(const Vector& params) const {
  double error = 0;
  for (const auto& measurementSeries : measurementSeriesVector_) {
    for (const auto& measurement : measurementSeries) {
      const double gravityCompensation = getGravityCompensation(params, measurement.jointPositionTicks_);
...
std::string GravityCompensation::getFilenameTrunk() const {
  return "gravity_compensation_" + std::to_string(motorVelocityAbs_) + "_" + std::to_string(turns_);
}

void GravityCompensation::toFiles(const Vector& params) const {
  for (unsigned int i = 0; i < n_; i++) {
    ...
    for (const auto& measurement : measurementSeriesVector_[i]) {
      ...
      file << getGravityCompensation(params, measurement.jointPositionTicks_) << ",";
...
void GravityCompensation::fromFiles() {
  ...
  for (unsigned int i = 0; i < n_; i++) {
    ...
    MeasurementSeries measurementSeries;
    int32_t jointPositionTicks = 0;
    double jointTorque = 0;
    double dummy = 0;
    double temperature = 0;
    while (file >> jointPositionTicks >> jointTorque >> dummy >> temperature) {
      measurementSeries.push_back(Measurement(jointPositionTicks, jointTorque, temperature));
    }
    measurementSeriesVector_.push_back(measurementSeries);
    file.close();
