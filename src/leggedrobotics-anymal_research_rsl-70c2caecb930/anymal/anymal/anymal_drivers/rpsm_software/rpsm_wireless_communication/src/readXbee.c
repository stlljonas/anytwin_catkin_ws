    ...
    if (fd < 0){
        ...
        if(tcgetattr(fd, tty)!=0){
            ...
            return -1;
                ...
                // No Timeout
                // Imediately return regardless of whether there is any data 
                tty->c_cc[VMIN]   =  1;
                tty->c_cc[VTIME]  =  1;
...
int xbee_read (int fd, uint8_t *buffer, int length) {
    ...
    byte = (uint8_t *)malloc(1);
    crcLeft = (uint8_t *)malloc(1);
    crcRight = (uint8_t *)malloc(1);
...
uint16_t convertToUint16 (uint8_t higher, uint8_t lower){
    ...
    uint16_t ret = ((uint16_t)higher << 8) | lower ;
...
float convertToFloat32 (uint8_t integer, uint8_t decimal){
    ...
    float ret = integer + (decimal / 100.0);
