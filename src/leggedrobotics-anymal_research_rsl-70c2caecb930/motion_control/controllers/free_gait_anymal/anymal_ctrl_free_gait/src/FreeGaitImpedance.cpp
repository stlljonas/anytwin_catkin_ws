...
void FreeGaitImpedance::setupControlModules() {
  ...
  contactDetector_.reset(new loco::ContactDetectorConstantDuringStance(*wholeBody_->getLegsPtr()));
  ...
  // foot placement strategy and torso control.
  ...
  motionController_.reset(new loco::ImpedanceAndVirtualModelControllerFreeGait(*wholeBody_, *getState().getAnymalModelPtr(),
                                                                               anymalModelDesired, *contactForceDistribution_));
}
