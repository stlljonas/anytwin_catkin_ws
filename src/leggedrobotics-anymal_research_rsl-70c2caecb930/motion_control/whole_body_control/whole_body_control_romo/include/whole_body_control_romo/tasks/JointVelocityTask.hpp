...
class JointVelocityTask : public JointMotionTask<ConcreteDescription_, RobotState_> {
 protected:
 ...
 public:
  explicit JointVelocityTask(WholeBodyState& wholeBodyState) : Base(wholeBodyState) {
  ...
  static std::unique_ptr<whole_body_control::Task> createTask(WholeBodyState& wholeBodyState) {
    return std_utils::make_unique<JointVelocityTask<ConcreteDescription_, RobotState_>>(wholeBodyState);
  }
  static std::string getTaskTypeName() { return "joint_velocity"; }
  ...
  bool update(double dt, int solutionSpaceDimension) override {
    ...
    for (unsigned int k = 0; k < nDim; ++k) {
      ...
      this->equalityConstraintTargetValues_(k) = this->pidGains_.proportionalGains_.getValue() * jointVelocityError;
    }
