...
FreeGaitActionServer::FreeGaitActionServer(ros::NodeHandle& nodeHandle, std::string name,
...
{
  if (!server_.isActive() || isBlocked_ || isInitializingNewGoal_) {
    return;
  }
  if (isIgnoringGoal_) {
    setPreempted();
...
void FreeGaitActionServer::goalCallback()
{
  ...
  if (goal->steps.empty()) {
    ...
    isIgnoringGoal_ = true;
  } else {
    isIgnoringGoal_ = false;
  }

  std::vector<Step> steps;
  for (const auto& stepMessage : goal->steps) {
    Step step;
...
void FreeGaitActionServer::publishFeedback()
{
  ...
  if (!executor_.getState().getRobotExecutionStatus()|| !executor_.getQueue().active()) {
