                                   ...
                                   EndEffectorTarget& endEffectorTarget)
{
  ...
  // Average Velocity.
  if(message.average_velocity > 0.0){
  ...
  // Make sure all gains are positive
  for (int i = 0; i < 3; i++){
    Kp(i) = std::abs(Kp(i));
    Kd(i) = std::abs(Kd(i));
    Kf(i) = std::abs(Kf(i));
                                   ...
                                   EndEffectorTrajectory& endEffectorTrajectory)
{
  ...
  endEffectorTrajectory.isImpedanceTrajectory_ = !message.impedance_control.empty();
  
  if (endEffectorTrajectory.isImpedanceTrajectory_) {
  ...
  for (const auto& point : message.trajectory.points) {
    if (!point.transforms.empty()) {
      ...
      MELO_WARN_STREAM("Could not read from ROS message, only position trajectories are supported for now.")
      break;
    }
  }
