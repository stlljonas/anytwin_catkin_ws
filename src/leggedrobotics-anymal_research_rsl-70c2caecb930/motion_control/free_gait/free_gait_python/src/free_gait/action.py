...
class ActionState:
    ...
    SUCCEEDED = 5      # Successfully executed.
    PREEMPTED = 6      # Preempted during execution.
    ...
    def to_text(action_state):
        if action_state == ActionState.ERROR:
            ...
            return 'Idle'
        elif action_state == ActionState.SUCCEEDED:
            return 'Succeeded'
        elif action_state == ActionState.PREEMPTED:
            return 'Preempted'
        else:
            return 'Undefined'
...
class ActionBase(object):
    ...
    def set_state(self, state):
        ...
        if state == self.state:
            return
        self.state = state
        if self.state == ActionState.INITIALIZED \
        or self.state == ActionState.PENDING \
        or self.state == ActionState.ACTIVE \
        or self.state == ActionState.IDLE:
            if self.feedback_callback:
                self.feedback_callback()
        elif self.state == ActionState.SUCCEEDED \
             or self.state == ActionState.PREEMPTED \
             or self.state == ActionState.ERROR:
    ...
    def wait_for_state(self, state):
        ...
        wait_for_state = WaitForState(self, state, self.timeout)
        wait_for_state.wait()
    ...
    def _send_goal(self):
        ...
        if self.goal is None:
            ...
            rospy.logerr("[Action Loader] No goal to send.")
    ...
    def _done_callback(self, status, result):
        ...
        if status == GoalStatus.SUCCEEDED:
            rospy.loginfo('Received goal status: ' + str(status) + '. Action will switch to state Succeeded.')
            self.set_state(ActionState.SUCCEEDED)
        elif status == GoalStatus.PREEMPTED or status == GoalStatus.RECALLED:
            rospy.loginfo('Received goal status: ' + str(status) + '. Action will switch to state Preempted.')
            self.set_state(ActionState.PREEMPTED)
        else:
            rospy.loginfo('Received goal status: ' + str(status) + '. Action will switch to state Error.')
            self.set_state(ActionState.ERROR)
...
class ContinuousAction(ActionBase):
    ...
    def _done_callback(self, status, result):
        ...
        if status == GoalStatus.SUCCEEDED:
            self.set_state(ActionState.SUCCEEDED)
        if status == GoalStatus.PREEMPTED or status == GoalStatus.RECALLED:
            self.set_state(ActionState.PREEMPTED)
        else:
...
class LaunchAction(ActionBase):
    ...
    def _stop(self):
        ...
        if os.path.exists(self.temp_launch_file.name):
