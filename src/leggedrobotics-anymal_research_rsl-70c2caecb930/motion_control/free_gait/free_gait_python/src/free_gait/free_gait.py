...
def parse_twist_forces(yaml_object):
    twist = geometry_msgs.msg.Twist()
    if 'force' in yaml_object:
        twist.linear = parse_vector(yaml_object['force'])
    return twist
...
def parse_translational_trajectory(joint_name, trajectory):
    ...
    for knot in trajectory['knots']:
        ...
        if 'acceleration' in knot:
          if 'force' in knot:
            raise Exception('Cannot specify both acceleration and force')
          twist_acceleration = parse_twist_accelerations(knot)
          point.accelerations.append(twist_acceleration)
        elif 'force' in knot:
          twist_acceleration = parse_twist_forces(knot)
          point.accelerations.append(twist_acceleration)
        output.points.append(point)
    return output
