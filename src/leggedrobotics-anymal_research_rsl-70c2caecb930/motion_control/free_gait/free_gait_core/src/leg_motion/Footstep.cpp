...
bool Footstep::compute(bool isSupportLeg)
{
  ...
  } else if (profileType_ == "trapezoid_xyz") {
    generateTrapezoidXYZKnots();
  } else {
...
{
  Pose transformTargetToWorld, transformBaseToWorld;
  if (!adapter.getFrameTransform(frameId_, transformTargetToWorld) || !adapter.getFrameTransform(adapter.getBaseFrameId(), transformBaseToWorld)) {
    return false;
  }
  const Eigen::Matrix4d poseWorldToTarget = transformTargetToWorld.getTransformationMatrix().inverse();
  const Eigen::Matrix4d poseBaseToWorld = transformBaseToWorld.getTransformationMatrix();
  Eigen::Matrix4d poseBaseToTarget = poseWorldToTarget * poseBaseToWorld;
  tranformBaseToTargetFrame_.getPosition() = Position(poseBaseToTarget.rightCols<1>().head<3>());
  tranformBaseToTargetFrame_.getRotation() = RotationQuaternion(RotationMatrix(poseBaseToTarget.topLeftCorner<3,3>()));
...
void Footstep::generateTrapezoidXYZKnots() {
  Position offsetAlongXinBaseFrame(-profileHeight_, 0.0, 0.0);
  Position offsetAlongYinBaseFrame(0.0, profileHeight_, 0.0);
  offsetAlongYinBaseFrame *= (static_cast<unsigned>(limb_) % 2 == 0) ? 1.0 : -1.0;
  Position offsetAlongXinTargetFrame  = tranformBaseToTargetFrame_.getRotation().rotate(offsetAlongXinBaseFrame);
  Position offsetAlongYinTargetFrame  = tranformBaseToTargetFrame_.getRotation().rotate(offsetAlongYinBaseFrame);
  ...
  // Knot 2.
  Position knot2 = start_ + 0.1 * (target_ - start_);
  knot2 += 0.75 * offsetAlongXinTargetFrame;
  knot2 += 0.75 * offsetAlongYinTargetFrame;
  knot2.z() = start_.z() + 0.75 * profileHeight_;
  values_.push_back(knot2.vector());

  // Knot 3.
  Position knot3 = start_ + 0.5 * (target_ - start_);
  knot3 += 0.5 * offsetAlongXinTargetFrame;
  knot3 += 1.75 * offsetAlongYinTargetFrame;
  knot3.z() = target_.z() +  0.75 * profileHeight_;
  values_.push_back(knot3.vector());

  // Knot 4.
  Position knot4 = start_ + 0.9 * (target_ - start_);
  knot4 += 0.1 * offsetAlongXinTargetFrame;
  knot4 += 0.75 * offsetAlongYinTargetFrame;
  knot4.z() = target_.z() + profileHeight_;
