...
bool PoseOptimizationSQP::optimize(Pose& pose)
{
  ...
  if (solverType_.compare("OSQP") == 0) {
    qpSolver.reset(new numopt_osqp::OperatorSplittingQuadraticProgramFunctionMinimizer());
  } else if (solverType_.compare("QUADPROG") == 0) {
    qpSolver.reset(new numopt_quadprog::ActiveSetFunctionMinimizer());
  } else {
    qpSolver.reset(new numopt_quadprog::ActiveSetFunctionMinimizer());
  }
