...
class EndEffectorMotionBase : public LegMotionBase
{
 public:
  ...
  /*!
   * Gets the impedance gains for impedance control motions.
   * @return the impedance gains.
   */
  virtual const Vector& getImpedanceGain(const ImpedanceControl& impedanceControlId) const;

  /*!
   * Gets the impedance gain frame ID for impedance control motions.
   * @return frame ID.
   */
  virtual const std::string& getImpedanceGainFrameId(const ImpedanceControl& impedanceControlId) const;

  /*!
   * Gets the feedforward friction norm for impedance control motions.
   * @return feedforward friction norm.
   */
  virtual double getFeedForwardFrictionNorm() const;

  /*!
   * Returns true if motion is impedance trajectory.
   * @return whether motion is impedance trajectory.
   */
