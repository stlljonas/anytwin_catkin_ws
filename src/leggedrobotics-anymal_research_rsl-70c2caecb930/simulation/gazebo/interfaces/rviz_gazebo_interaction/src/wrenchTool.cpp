...
void WrenchTool::onInitialize() {
  ...
  auto gazeboCollisionsMessage = ros::topic::waitForMessage<gazebo_msgs::LinkStates>("/gazebo/link_states", ros::Duration(10, 0));
  if (gazeboCollisionsMessage) {
    gazeboSDFCollisions_ = gazeboCollisionsMessage->name;
  }
  deactivate();
}
...
bool WrenchTool::sendWrenchToGazebo(const Ogre::Vector3& force) {
  ...
  srv.request.body_name = currentRobotLink_->getCollisionParentLinkName(gazeboSDFCollisions_);
