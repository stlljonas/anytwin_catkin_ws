...
void AnybotGazeboPlugin<ConcreteDescription_>::Load(physics::ModelPtr model, sdf::ElementPtr sdf) {
  ...
  if (!ros::isInitialized()) {
    MELO_FATAL_STREAM("[" << robotName_
                          << "GazeboPlugin::Load] A ROS node for Gazebo has not been initialized, unable to load plugin. Load the Gazebo "
  ...
  MELO_INFO_STREAM("[" << robotName_ << "GazeboPlugin::Load] Starting gazebo_ros_control plugin in namespace: '"
  ...
  model_->GetWorld()->Physics()->SetRealTimeUpdateRate(realTimeUpdateRate_);
  model_->GetWorld()->Physics()->SetMaxStepSize(maxStepSize_);
  ...
  updateConnection_ = event::Events::ConnectWorldUpdateBegin(boost::bind(&AnybotGazeboPlugin::updateCb, this));
}
...
std::string AnybotGazeboPlugin<ConcreteDescription_>::getUrdfRobotDescription(const std::string& paramName) const {
  ...
  while (urdfString.empty()) {
    ...
    if (nodeHandle_->searchParam(paramName, searchParamName)) {
      MELO_INFO_STREAM("[" << robotName_ << "GazeboPlugin::getUrdfRobotDescription] Waiting for model URDF in parameter " << searchParamName
...
void AnybotGazeboPlugin<ConcreteDescription_>::updateCb() {
  ...
  if (!stateHasToBePublished()) return;
...
void AnybotGazeboPlugin<ConcreteDescription_>::readSimulation() {
  for (const auto actuatorKey : ConcreteDescription::getActuatorKeys()) {
    ...
    jointVelocityFilter_[actuatorEnum].setSamplingTime(updateSimTimeStep_, jointVelocityFilter_[actuatorEnum].getFilteredValue());
    ...
    double currVel = jointVelocityFilter_[actuatorEnum].advance((currJointPositions_[actuatorEnum] - prevJointPositions_[actuatorEnum]) /
                                                                updateSimTimeStep_);

    currJointVelocities_[actuatorEnum] = currVel;

    // same for velocities
    jointAccelerationFilter_[actuatorEnum].setSamplingTime(updateSimTimeStep_, jointAccelerationFilter_[actuatorEnum].getFilteredValue());
    ...
    updateActuatorReading(actuatorEnum, currJointPositions_[actuatorEnum], currJointVelocities_[actuatorEnum],
                          currJointAccelerations_[actuatorEnum], currJointTorques_[actuatorEnum]);
...
template <typename ConcreteDescription_>
any_measurements::Wrench AnybotGazeboPlugin<ConcreteDescription_>::createContactWrenchMessage(
    const typename ConcreteDescription::ContactEnum& contactEnum) {
...
void AnybotGazeboPlugin<ConcreteDescription_>::publishTwist() {
  ...
  if (twistPublisher_) {
    if (twistPublisher_->getNumSubscribers() == 0 && twistPublisherThrottled_.getNumSubscribers() == 0) {
      return;
    }
  }
...
void AnybotGazeboPlugin<ConcreteDescription_>::setRobotPose(const geometry_msgs::Pose& pose) {
  ...
  const ignition::math::Pose3d poseGazebo(
      ignition::math::Vector3d(pose.position.x, pose.position.y, pose.position.z),
      ignition::math::Quaterniond(pose.orientation.w, pose.orientation.x, pose.orientation.y, pose.orientation.z));
  model_->SetWorldPose(poseGazebo);
  model_->SetLinearVel(ignition::math::Vector3d());
  model_->SetAngularVel(ignition::math::Vector3d());
#else
  const math::Pose poseGazebo(math::Vector3(pose.position.x, pose.position.y, pose.position.z),
                              math::Quaternion(pose.orientation.w, pose.orientation.x, pose.orientation.y, pose.orientation.z));
  model_->SetWorldPose(poseGazebo);
...
template <typename ConcreteDescription_>
bool AnybotGazeboPlugin<ConcreteDescription_>::setRobotPoseCb(any_gazebo_msgs::SetRobotPoseRequest& req,
