...
class AnybotGazeboPlugin : public ModelPlugin {
 ...
 public:
  // Constructor.
  explicit AnybotGazeboPlugin(const std::string& robotName) : robotName_(robotName) {}
  ...
  virtual bool setRobotPoseCb(any_gazebo_msgs::SetRobotPoseRequest& req, any_gazebo_msgs::SetRobotPoseResponse& res);
 ...
 protected:
  ...
  virtual void updateActuatorReading(const ActuatorEnum& actuatorEnum, const double jointPosition, const double jointVelocity,
                                     const double jointAcceleration, const double jointTorque) = 0;
  ...
  any_measurements::Time getTime() { return any_measurements::Time(lastStartUpdateSimTime_.sec, lastStartUpdateSimTime_.nsec); }
  ...
  // Specifies in Hz the number of physics updates that will be attempted per second. If this number is set to zero, it will run as fast as
  // it can.
  double realTimeUpdateRate_ = 1000;
  // Specifies the time duration in seconds of each physics update step.
  double maxStepSize_ = 0.001;
  // Simulate the state estimator. If true, this plugin will publish all estimator output containing ground truth data.
  bool simulateEstimator_ = false;
  // If the state estimator is not simulated and this is set to true, the plugin will still publish a groundtruth anymal state
  ...
  JointEnumContainer<basic_filters::FirstOrderFilterD> jointVelocityFilter_;
  JointEnumContainer<basic_filters::FirstOrderFilterD> jointAccelerationFilter_;
