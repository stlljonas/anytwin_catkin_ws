...
bool DocumentHandleXML::create(const std::string& path, DocumentMode mode, const std::string& rootName) {
  ...
  if (writeable() && !success) {
    ...
    MELO_ERROR(
        "[tinyxml_tools::DocumentHandle] create(...) was not able to read the file '%s', and cannot create a new file as a non-writeable "
        "mode was chosen",
        filepath_.c_str());
    return false;
  }
...
bool DocumentHandleXML::merge(const std::string& path, MergeMode mode) {
  return merge(path, mode, false);
}
