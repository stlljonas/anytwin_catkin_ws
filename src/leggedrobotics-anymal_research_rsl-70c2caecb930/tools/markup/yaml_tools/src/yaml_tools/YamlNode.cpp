...
YamlNode::YamlNode(const Impl& impl, std::string nestedKey) : impl_(impl), nestedKey_(std::move(nestedKey)) {}

YamlNode::YamlNode(Type type) : impl_(Impl(type)) {}

YamlNode::Impl& YamlNode::getImpl() {
  return impl_;
}

const YamlNode::Impl& YamlNode::getImpl() const {
  return impl_;
}
...
std::string YamlNode::toString(EmitterFormat format) const {
  try {
    if (impl_.IsNull()) {
      return std::string();
    }
    return internal::nodeToOrderedString(*this, format);
  } catch (const YAML::Exception& exception) {
...
void YamlNode::pushBack(const YamlNode& node) {
  impl_.push_back(node.getImpl());
}

Iterator YamlNode::begin() {
  return Iterator(this, true);
}

ConstIterator YamlNode::begin() const {
  return ConstIterator(this, true);
}

Iterator YamlNode::end() {
  return Iterator(this, false);
}

ConstIterator YamlNode::end() const {
