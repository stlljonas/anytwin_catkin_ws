...
class TestYamlNodeSequence : public ::testing::Test {
 ...
 protected:
  ...
  const std::pair<std::string, int> integer_ = {"integer", 2};
  ...
  const std::string yamlString_ =
      ...
      "- name: string\n"
      "  value: hello";
  const std::string yamlStringOneLine_ =
      "[{name: bool, value: true}, {name: integer, value: 2}, {name: double, value: 4.5}, {name: string, value: hello}]";
...
TEST_F(TestYamlNodeSequence, writeToOneLineString) {  // NOLINT
  EXPECT_EQ(yamlStringOneLine_, yamlNode_.toString(yaml_tools::YamlNode::EmitterFormat::ONE_LINE));
}
...
TEST_F(TestYamlNodeSequence, iteratorAccess) {  // NOLINT
  EXPECT_EQ(bool_.second, (*yamlNode_.begin())["value"].as<bool>());
}

TEST_F(TestYamlNodeSequence, iteratorPostIncrement) {  // NOLINT
  EXPECT_EQ(bool_.second, (*yamlNode_.begin()++)["value"].as<bool>());
}

TEST_F(TestYamlNodeSequence, iteratorPreIncrement) {  // NOLINT
  EXPECT_EQ(integer_.second, (*++yamlNode_.begin())["value"].as<int>());
}
...
TEST_F(TestYamlNodeSequence, constIteratorBasedForLoop) {  // NOLINT
  ...
  EXPECT_EQ(yamlString_, yamlNode.toString());
}
