...
class TestYamlNodeMap : public ::testing::Test {
 ...
 protected:
  ...
  const std::string yamlString_ =
      ...
      "double: 4.5\n"
      "string: hello";
  const std::string yamlStringAlphabetical_ =
      "bool: true\n"
      "double: 4.5\n"
      "integer: 2\n"
      "string: hello";
  const std::string yamlStringOneLine_ = "{bool: true, double: 4.5, integer: 2, string: hello}";
...
TEST_F(TestYamlNodeMap, writeToOneLineString) {  // NOLINT
...
TEST_F(TestYamlNodeMap, iteratorAccess) {  // NOLINT
  EXPECT_EQ(bool_.second, (*yamlNode_.begin()).second.as<bool>());
}

TEST_F(TestYamlNodeMap, iteratorPostIncrement) {  // NOLINT
  EXPECT_EQ(bool_.second, (*yamlNode_.begin()++).second.as<bool>());
}

TEST_F(TestYamlNodeMap, iteratorPreIncrement) {  // NOLINT
  EXPECT_EQ(double_.second, (*++yamlNode_.begin()).second.as<double>());
}
...
TEST_F(TestYamlNodeMap, constIteratorBasedForLoop) {  // NOLINT
  ...
  EXPECT_EQ(yamlStringAlphabetical_, yamlNode.toString());
}
