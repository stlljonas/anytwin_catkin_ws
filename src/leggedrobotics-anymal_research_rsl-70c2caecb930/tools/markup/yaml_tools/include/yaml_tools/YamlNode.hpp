...
class YamlNode {
 public:
  ...
  //! Enum to configure emitter
  enum class EmitterFormat { DEFAULT = 0, ONE_LINE };
 ...
 public:
  ...
  YamlNode() = default;
  //! Constructor creating an empty node but setting its type.
  explicit YamlNode(Type type);
   ...
   * Throws a yaml_tools::Exception if unsuccessful.
   * @param format Emitter format to use for YAML emitting.
   * @return YAML node in string format.
   */
  std::string toString(EmitterFormat format = EmitterFormat::DEFAULT) const;
   ...
   * @param path Absolute path of the file.
   */
  void toFile(const std::string& path, EmitterFormat format = EmitterFormat::DEFAULT) const;
  ...
  /*!
   * Add a YAML node to the YAML node.
   * Applicable YAML node types: Sequence.
   * Throws a yaml_tools::Exception if unsuccessful.
   * @param node Node to add.
   */
  void pushBack(const YamlNode& node);

  /*!
   * Add a scalar to the YAML node.
   * Applicable YAML node types: Sequence.
   * Throws a yaml_tools::Exception if unsuccessful.
   * @tparam Scalar Type of the scalar.
   ...
   */
  template <typename Scalar>
  void pushBack(const Scalar& scalar) {
    try {
      // Call operator=(..) to make use of the contained workaround.
      YamlNode node;
      node = scalar;
      pushBack(node);
