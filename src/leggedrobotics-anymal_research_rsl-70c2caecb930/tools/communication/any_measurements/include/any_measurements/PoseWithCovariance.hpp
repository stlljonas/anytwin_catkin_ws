...
struct PoseWithCovariance : public Pose {
 public:
  ...
  PoseWithCovariance() : Pose(), covariance_(CovarianceMatrix::Zero()) {}

  PoseWithCovariance(const Time& time, const kindr::HomTransformQuatD& pose, const CovarianceMatrix& cov)
      : Pose(time, pose), covariance_(cov) {}
