...
struct TwistWithCovariance : public Twist {
 public:
  ...
  TwistWithCovariance() : Twist(), covariance_(CovarianceMatrix::Zero()) {}

  TwistWithCovariance(const Time& time, const kindr::TwistLocalD& twist, const CovarianceMatrix& cov)
      : Twist(time, twist), covariance_(cov) {}
