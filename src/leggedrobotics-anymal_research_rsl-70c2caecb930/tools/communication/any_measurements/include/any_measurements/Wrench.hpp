...
struct Wrench {
 public:
  ...
  Wrench(const Time& time, const kindr::Force3D& force, const kindr::Torque3D& torque) : time_(time), wrench_(force, torque) {}

  Wrench(const Time& time, const kindr::WrenchD& wrench) : time_(time), wrench_(wrench) {}
