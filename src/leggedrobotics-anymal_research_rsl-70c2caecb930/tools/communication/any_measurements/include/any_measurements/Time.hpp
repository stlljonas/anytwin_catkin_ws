...
struct Time {
 public:
  ...
  Time(const int64_t seconds, const int64_t nanoSeconds) : time_{seconds, nanoSeconds} {}

  /*!
   * Constructor using seconds floating point value
   * @param seconds   Seconds to construct time from
   */
  explicit Time(const double seconds) : Time() { fromSeconds(seconds); }

  /*!
   * Constructor using linux timespec structure
   * @param other     reference to the object to be copied
   */
  explicit Time(const timespec& other) : time_(other) {}
  ...
  explicit Time(const std::chrono::time_point<ClockType>& other) : time_() {
    *this = other;
  }
   ...
   */
  bool operator==(const Time& other) const { return (time_.tv_sec == other.time_.tv_sec && time_.tv_nsec == other.time_.tv_nsec); }
  ...
  inline int64_t seconds() const { return time_.tv_sec; }

  inline void setSeconds(const int64_t seconds) { time_.tv_sec = seconds; }

  inline int64_t nanoSeconds() const { return time_.tv_nsec; }

  inline void setNanoSeconds(const int64_t nanoSeconds) { time_.tv_nsec = nanoSeconds; }

  inline double toSeconds() const { return static_cast<double>(seconds()) + 1e-9 * static_cast<double>(nanoSeconds()); }
  ...
  inline int64_t toNanoSeconds() const { return static_cast<int64_t>(1e9) * seconds() + nanoSeconds(); }
  ...
  /*!
   * Return the current time as a formatted string in the local timezone.
   * @param format e.g. "%Y-%m-%d_%H-%M-%S"
   * @return formatted date/time string
   */
  inline std::string format(const std::string& format) { return formatTimezone(format, ""); }

  /*!
   * Return the current time as a formatted string.
   * @param format e.g. "%Y-%m-%d_%H-%M-%S"
   * @param timezone e.g. "UTC" or "Europe/Zurich", if empty use the local timezone
   * @return formatted date/time string
   */
  inline std::string formatTimezone(const std::string& format, const std::string& timezone = "UTC") {
    struct tm* timeInfo;
    char buffer[128];
    ...
    timeInfo = std::localtime(&(time_.tv_sec));
   ...
   */
  static Time Now() {  // NOLINT(readability-identifier-naming)
    Time time;
   ...
   */
  static Time NowWallClock() {  // NOLINT(readability-identifier-naming)
    Time time;
   ...
   * @return          current wall clock time point
   */
  static Time NowRaw() {  // NOLINT(readability-identifier-naming)
    Time time;
   ...
   * @return          current wall clock time point
   */
  static Time NowCpuProcess() {  // NOLINT(readability-identifier-naming)
    Time time;
   ...
   * @return          current wall clock time point
   */
  static Time NowCpuThread() {  // NOLINT(readability-identifier-naming)
    Time time;
  ...
  inline bool isWallClock() const {
    return (time_.tv_sec > static_cast<time_t>(1e9));  // if the time was taken using the monotonic clock, the computer would need to run
                                                       // since several thousand days to get to this value...
  }
  ...
  Time getElapsedTimeMonotonicRaw() const { return (NowRaw() - *this); }

  Time getElapsedTimeMonotonic() const { return (Now() - *this); }

  Time getElapsedTimeWallClock() const { return (NowWallClock() - *this); }

  Time getElapsedTimeCpuProcess() const { return (NowCpuProcess() - *this); }

  Time getElapsedTimeCpuThread() const { return (NowCpuThread() - *this); }
  ...
  timespec& getImpl() { return time_; }

  const timespec& getImpl() const { return time_; }
