...
class SubscriberRosOptions : public cosmo::SubscriberOptions<Msg_> {
 public:
   ...
   */
  SubscriberRosOptions(const std::string& topic, const SubscriberCallback& callback, const ros::NodeHandle& nodeHandle = ros::NodeHandle())
      : BaseType(topic, callback), nodeHandle_(nodeHandle), rosTransportHints_(), rosQueueSize_(100), tryRosResubscribing_(true) {}
