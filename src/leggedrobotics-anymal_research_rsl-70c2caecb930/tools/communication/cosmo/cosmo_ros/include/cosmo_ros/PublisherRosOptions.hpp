...
class PublisherRosOptions : public cosmo::PublisherOptions {
 public:
   ...
   */
  explicit PublisherRosOptions(const std::string& topic, const ros::NodeHandle& nodeHandle = ros::NodeHandle())
      : PublisherOptions(topic), nodeHandle_(nodeHandle), autoPublishRos_{false}, rosQueueSize_{100}, rosLatch_{false} {}

  ~PublisherRosOptions() override = default;
