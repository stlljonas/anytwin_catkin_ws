...
class Subscriber : public Communicator<MessageType_, false, Alignment_> {
 public:
  ...
  Subscriber() = delete;
  Subscriber(const std::string& topic, const SubscriberCallback& callback) : Subscriber(std::make_shared<Options>(topic, callback)) {}

  explicit Subscriber(const OptionsPtr& options)
      : BaseType(options),
        options_(options),
        active_{false},
        thread_(),
   ...
   */
  virtual void stop(const bool wait) {
    active_ = false;
   ...
   *             maxLockTime
   */
  virtual bool receive(const std::chrono::microseconds& maxLockTime) {
    ...
    smLock_.lock();
    bool hasNewMessage =
        this->smCnd_->timed_wait(smLock_, abs_time, [=]() { return (*(this->smCounter_) > localCounter_ || !this->active_); });
    if (!this->active_) {
      unlockMutex();
  ...
  inline bool receive() { return receive(std::chrono::microseconds{200}); }
   ...
   */
  unsigned long int getMessageCounter() const { return localCounter_; }
 ...
 protected:
  ...
  std::thread thread_{};
  std::atomic<unsigned long int> localCounter_{0};
