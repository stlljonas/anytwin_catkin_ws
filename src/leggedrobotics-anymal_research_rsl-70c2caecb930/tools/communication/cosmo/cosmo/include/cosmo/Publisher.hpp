...
class Publisher : public Communicator<MessageType_, true, Alignment_> {
 public:
  ...
  Publisher() = delete;
  explicit Publisher(const std::string& topic) : Publisher(std::make_shared<Options>(topic)) {}

  explicit Publisher(const PublisherOptionsPtr& options) : BaseType(options), options_(options) {}
   ...
   */
  bool publish(const MessageType_& message, const std::chrono::microseconds& maxLockTime) {
    bool sendSuccessful = false;
    {
      ...
      boost::interprocess::scoped_lock<boost::interprocess::interprocess_sharable_mutex> lock(*(this->smMutex_),
  ...
  inline bool publish(const MessageType_& message) { return publish(message, std::chrono::microseconds{50}); }
