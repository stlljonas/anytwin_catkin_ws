...
class RBF1D {
 public:
  ...
  //! Destructor
  virtual ~RBF1D() = default;

  /*! Compute function f(x), so that f(x_i) = f_i. The form of f is:
   * @param xInput	the centers x_i
   * @param fInput	the function values f_i
   */
  virtual void setRBFData(const std::vector<double>& xInput, const std::vector<double>& fInput);
  virtual void setRBFData(const Eigen::VectorXd& xInput, const Eigen::VectorXd& fInput);
 ...
 protected:
   ...
   */
  static double wrapToRange(double x, double min, double max);
};
