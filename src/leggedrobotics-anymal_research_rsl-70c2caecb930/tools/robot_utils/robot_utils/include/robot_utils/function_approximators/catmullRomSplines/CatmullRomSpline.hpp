...
/**
        This class is used to represent generic trajectories. The class parameter T can be anything that provides basic operation such as
   addition and subtraction. We'll define a trajectory that can be parameterized by a one-d parameter (called t). Based on a set of knots (
   tuples <t, T>), we can evaluate the trajectory at any t, through interpolation. This is not used for extrapolation. Outside the range of
   the knots, the closest known value is returned instead.
*/
...
class CatmullRomSpline {
 protected:
  ...
  /**
          This method returns the index of the first knot whose value is larger than the parameter value t. If no such index exists (t is
     larger than any of the values stored), then values.size() is returned.
  */
 ...
 public:
  ...
  CatmullRomSpline(void) { lastIndex = 0; }
  ...
  virtual ~CatmullRomSpline(void) { clear(); }
  ...
  */
  ...
  T evaluate(double t) { return evaluate_catmull_rom(t); }
  ...
  */
  const T& getKnotValue(int i) const { return values[i]; }

  T& getKnotValue(int i) { return values[i]; }
  ...
  */
  int getKnotCount() const { return tValues.size(); }
