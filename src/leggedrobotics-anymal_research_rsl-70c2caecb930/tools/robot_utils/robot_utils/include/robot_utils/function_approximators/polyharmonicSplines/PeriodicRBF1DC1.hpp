...
//! Function approximation of a periodic function with radial basis functions (polyharmonic splines) with C1 continuity
/*!
 * Compute function f(x), so that f(x_i) = f_i. The form of f is:
 * f = sum_i w_i * F_i(r) + a + b*x + c*x*x, where F_i are basis functions, one for each (x_i, f_i) pair.
 ...
 * whereby f(x_0)=f(x_n-1), f'(x_0)=f'(x_n-1), so that f(x) has C1 continuity
 * The basis function is F_i(r_i) = r_i*r_i*log(r_i) with r_i = (x_i-x)*(x_i-x)
 *
 */
class PeriodicRBF1DC1 : public RBF1D {
 public:
  //! Constructor
  PeriodicRBF1DC1() = default;

  //! Destructor
  ~PeriodicRBF1DC1() override = default;
