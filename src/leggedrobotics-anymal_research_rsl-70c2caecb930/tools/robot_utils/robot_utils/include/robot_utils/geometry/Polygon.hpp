...
class Polygon {
 public:
  ...
  explicit Polygon(VertexList&& vertices, robot_utils::geometry::VertexOrder vertexOrder = robot_utils::geometry::VertexOrder::Undefined);
  ...
  bool isPointInPolygon(const Position2d& point, double tol = 1e-6) const;
  ...
  bool computeClosestPointOnPolygon(Position2d& pointOnPolygon, const Position2d& point) const;

 protected:
  virtual bool isPointInPolygonInternal(const Position2d& point, double tol) const;
