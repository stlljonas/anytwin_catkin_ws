...
class OptimizationNelderMead {
 ...
 public:
  ...
  bool optimize(double& fOpt, Eigen::VectorXd& xOpt, const Eigen::VectorXd& x0, const Fcn& fcn, bool minimize = true,
                long int maxIterations = -1, double maxFValueDiff = 1e-4, double maxVertexDiff = 1e-4) {
    // Check Input.
    if (xOpt.size() != x0.size()) {
      std::cout << "Parameter vector (" << xOpt.size() << "x1) and initial guess (" << x0.size() << "x1) have different sizes!"
                << std::endl;
      return false;
    }
