...
struct InterpolationTraits {
  ...
  static_assert(std::is_same<ValueType, DifferenceType>::value,
                "By default, InterpolationTraits assumes that the DifferenceType (as defined by DerivativeTraits<ValueType>) is the same "
                "as the ValueType. If this is not the case, you probably need to define a speicif");
