 ...
 */
TEST(kinematicsTest, getPoseEndToReferenceFromTwistReferenceToStartInStartFrame_smallRotations) {  // NOLINT
  double timeHorizon;
  ...
  kindr::RotationQuaternionPD rotationStartToReference =
      kindr::RotationQuaternionPD(kindr::EulerAnglesZyxPD(M_PI / 4, -M_PI / 3, M_PI / 6));
  ...
  for (unsigned int testIt = 0u; testIt < 4; testIt++) {
    ...
    kindr::Position3D realPositionReferenceToEndInReferenceFrame =
        positionReferenceToStartInReferenceFrame +
    ...
    kindr::TwistLocalD twistReferenceToStartInStartFrame(linearVelocityReferenceToStartInStartFrame,
                                                         angularVelocityReferenceToStartInStartFrame);
    kindr::HomTransformQuatD predictedPoseEndToReference = robot_utils::getPoseEndToReferenceFromTwistReferenceToStartInStartFrame(
        poseStartToReference, twistReferenceToStartInStartFrame, timeHorizon);

    KINDR_ASSERT_DOUBLE_MX_EQ(realPoseEndToReference.getTransformationMatrix(), predictedPoseEndToReference.getTransformationMatrix(),
                              1.0e-10, msg);
  }
};
 ...
 */
TEST(kinematicsTest, getApproximatedPoseEndToReferenceFromTwistReferenceToStartInStartFrame_radialPeriodicity) {  // NOLINT
 ...
 */
TEST(kinematicsTest, getApproximatedPoseEndToReferenceFromTwistReferenceToStartInStartFrame_general) {  // NOLINT
  ...
  kindr::Velocity3D linearVelocityReferenceToStartInStartFrame(
      3.4 * kindr::RotationMatrixPD(rotationStartToReference).matrix().row(0).transpose());

  // Angular velocity aligned with z-axis of reference frame and arbitrary magnitude
  kindr::LocalAngularVelocityD angularVelocityReferenceToStartInStartFrame(
      0.8 * kindr::RotationMatrixPD(rotationStartToReference).matrix().row(2).transpose());
  ...
  kindr::Position3D realPositionReferenceToEndInReferenceFrame =
      positionReferenceToStartInReferenceFrame +
      linearVelocityReferenceToStartInStartFrame.norm() * timeHorizon / (M_PI / 2) *
          kindr::Position3D(1.0, 1.0, 0.0);  // This term scales with time divided by the rotation angle
 ...
 */
TEST(kinematicsTest, getApproximatedPoseEndToReferenceFromTwistReferenceToStartInStartFrame) {  // NOLINT
  ...
  kindr::Position3D approximatedPostitionReferenceToEndInReferenceFrame =
      positionReferenceToStartInReferenceFrame +
  ...
  kindr::TwistLocalD twistReferenceToStartInStartFrame(linearVelocityReferenceToStartInStartFrame,
                                                       angularVelocityReferenceToStartInStartFrame);
  kindr::HomTransformQuatD predictedPoseEndToReference =
      robot_utils::getApproximatedPoseEndToReferenceFromTwistReferenceToStartInStartFrame(poseStartToReference,
                                                                                          twistReferenceToStartInStartFrame, timeHorizon);

  std::string msg;
  KINDR_ASSERT_DOUBLE_MX_EQ(approximatedPoseEndToReference.getTransformationMatrix(), predictedPoseEndToReference.getTransformationMatrix(),
                            1.0e-10, msg);
};
