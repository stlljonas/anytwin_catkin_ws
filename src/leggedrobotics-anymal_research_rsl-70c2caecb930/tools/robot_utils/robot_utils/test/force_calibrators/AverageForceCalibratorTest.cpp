...
volatile static bool g_run = true;

void run(robot_utils::AverageForceCalibrator* calibration) {  // NOLINT
...
TEST(AverageForceCalibratorTest, DISABLED_calibrate) {  // NOLINT
  robot_utils::AverageForceCalibrator::Config config;
  config.numSamples_ = 2;
  config.minNumGoodSamples_ = 2;
  robot_utils::AverageForceCalibrator calibration("lf_foot", config);
...
TEST(AverageForceCalibratorTest, calibrateSimple) {  // NOLINT
  ...
  robot_utils::AverageForceCalibrator calibrator("lf_foot", config);
...
TEST(AverageForceCalibratorTest, command) {  // NOLINT
  double timeStep = 0.01;
  kindr::WrenchD wrenchOffset;
  robot_utils::AverageForceCalibrator::Config config;
  ...
  config.alphaFilterForce_ = 1.0;
  robot_utils::AverageForceCalibrator calibrator("lf_foot", config);
  robot_utils::ForceCalibratorCommand command;
