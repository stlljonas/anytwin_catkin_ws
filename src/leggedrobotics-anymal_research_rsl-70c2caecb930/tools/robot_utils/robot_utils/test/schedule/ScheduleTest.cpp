...
TEST(Schedule, construct) {  // NOLINT
  robot_utils::Schedule<double> schedule(100.0);
  schedule.addProfile(std::shared_ptr<robot_utils::Profile<double>>(new robot_utils::ProfileStep<double>(10.0, 1.0)));
  schedule.addProfile(std::shared_ptr<robot_utils::Profile<double>>(new robot_utils::ProfileStep<double>(20.0, 2.0)));
  schedule.addProfile(new robot_utils::ProfileStep<double>(30.0, 3.0));
...
TEST(Schedule, rampProfile) {  // NOLINT
  robot_utils::ProfileRamp<double> profile(10.0, 20.0, 5.0);
...
TEST(Schedule, logChirpUpSweepProfile) {  // NOLINT
  robot_utils::ProfileLogChirpUpSweep<double> profile(10.0, 20.0, 1.0, 20.0, 5.0);
  EXPECT_EQ(0.0, profile.getStartTime());
