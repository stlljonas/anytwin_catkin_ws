 ...
 *
 */
kindr::HomTransformQuatD getPoseEndToReferenceFromTwistReferenceToStartInStartFrame(
    const kindr::HomTransformQuatD& poseStartToReference, const kindr::TwistLocalD& twistReferenceToStartInStartFrame,
  ...
  if (areNear(angle, 0.0)) {  // Check whether we need to take the limit of the analytical solution.
    ...
    kindr::Position3D positionReferenceToEndInReferenceFrame =
        kindr::Position3D(poseStartToReference.getPosition().vector() +
                          timeHorizonInSeconds * rotationStartToReference.matrix() * linearVelocityReferenceToStartInStartFrame.vector());
    ...
    kindr::Position3D positionReferenceToEndInReferenceFrame = kindr::Position3D(
        poseStartToReference.getPosition().vector() +
        rotationStartToReference.matrix() *
            (Eigen::Matrix3d::Identity() * timeHorizonInSeconds + 1 / angularSpeed * (1.0 - std::cos(angle)) * rotationAxisSkew +
             (timeHorizonInSeconds - 1.0 / angularSpeed * std::sin(angle)) * rotationAxisSkew * rotationAxisSkew) *
            linearVelocityReferenceToStartInStartFrame.vector());

    kindr::RotationMatrixPD rotationEndToReference =
        rotationStartToReference * kindr::RotationMatrixPD(kindr::AngleAxisD(angle, rotationAxis.vector()));
 ...
 *
 * finalPose = initialPose + initialOrientation * eulerVectorToRotationMatrix(t*angularVelocityInMovingFrame) * linearVelocityInMovingFrame
 * * t;
 *
 */
kindr::HomTransformQuatD getApproximatedPoseEndToReferenceFromTwistReferenceToStartInStartFrame(
    const kindr::HomTransformQuatD& poseStartToReference, const kindr::TwistLocalD& twistReferenceToStartInStartFrame,
    double timeHorizonInSeconds) {
  const kindr::RotationMatrixPD& rotationStartToReference = kindr::RotationMatrixPD(poseStartToReference.getRotation());
  const kindr::Velocity3D& linearVelocityReferenceToStartInStartFrame = twistReferenceToStartInStartFrame.getTranslationalVelocity();
  ...
  if (robot_utils::areNear(angularSpeed, 0.0)) {  // Avoid a division by zero
    ...
    kindr::Position3D positionReferenceToEndInReferenceFrame =
        kindr::Position3D(poseStartToReference.getPosition().vector() +
                          timeHorizonInSeconds * rotationEndToReference.matrix() * linearVelocityReferenceToStartInStartFrame.vector());
