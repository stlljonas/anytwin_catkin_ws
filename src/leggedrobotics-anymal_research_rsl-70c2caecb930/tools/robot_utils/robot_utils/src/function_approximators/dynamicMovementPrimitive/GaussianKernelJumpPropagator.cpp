...
GaussianKernelJumpPropagator::GaussianKernelJumpPropagator() : currentTime_(0.0) {}

GaussianKernelJumpPropagator::~GaussianKernelJumpPropagator() = default;
 ...
 */
bool GaussianKernelJumpPropagator::loadGaussianKernel(TiXmlElement* pElem, dmp::GaussianKernel* desiredTrajectory) {
  ...
  if (pElem->QueryDoubleAttribute("canSysCutOff", &canSysCutOff) != TIXML_SUCCESS) {
    printf("Could not find parameter canSysCutOff!\n");
    return false;
  }

  if (pElem->QueryBoolAttribute("exponentiallySpaced", &exponentiallySpaced) != TIXML_SUCCESS) {
    printf("Could not find parameter exponentiallySpaced!\n");
    return false;
  }

  desiredTrajectory->initialize(numBasisFunctions, activation, exponentiallySpaced, canSysCutOff);
 ...
 */
bool GaussianKernelJumpPropagator::loadMovement(const TiXmlHandle& hTrajectory) {
  double maxDuration;
  bool velocityMode;
  ...
  if (pElem->QueryDoubleAttribute("maxDuration", &maxDuration) != TIXML_SUCCESS) {
    printf("Could not find parameter maxDuration!\n");
    return false;
  }

  if (pElem->QueryBoolAttribute("velocityMode", &velocityMode) != TIXML_SUCCESS) {
