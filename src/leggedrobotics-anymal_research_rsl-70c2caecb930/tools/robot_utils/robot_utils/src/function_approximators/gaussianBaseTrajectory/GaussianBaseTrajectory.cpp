...
GaussianBaseTrajectory::GaussianBaseTrajectory() = default;
...
void GaussianBaseTrajectory::initialize(const Parameters& par, double timeHorizon, double dt) {
  ...
  setTimeStep(dt);
  setSigmaOfBasisFunctions(timeBetweenFirstAndLastParametrizedBaseFunction_ / (2.0 * (numberOfBasisfunctions_ - 1)));
...
double GaussianBaseTrajectory::getValueAtTimeFromNow(double time, double dt) const {
  return values_[static_cast<size_t>(time / dt)];
}
...
void GaussianBaseTrajectory::setNumberOfBasisFunctions(int numberOfBasisFunctions) {
...
void GaussianBaseTrajectory::setParameters(const Parameters& parameters) {
  parameters_.clear();
  parameters_.push_back(parameters[0] + (parameters[0] - parameters[2]));
  parameters_.push_back(parameters[0] + (parameters[0] - parameters[1]));
...
double GaussianBaseTrajectory::getValues(double& value, double& derivativeValue, double& secondDerivativeValue, double t) const {
  ...
  generateBasisFunctionVectors(baseFunctionVector, baseFunctionDerivativeVector, baseFunctionSecondDerivativeVector, t, sigma_,
                               timeBetweenFirstAndLastParametrizedBaseFunction_, numberOfBasisfunctions_);
...
void GaussianBaseTrajectory::generateBasisFunctionVectors(BaseValues& baseValues, BaseValues& baseDerivatives,
                                                          BaseValues& baseSecondDerivatives, double time, double sigma,
                                                          double timeBetweenFirstAndLastParametrizedBaseFunction,
                                                          int numberOfBasisFunctions) const {
  //  BaseValues output;
  double mu;
  double muSpacing = timeBetweenFirstAndLastParametrizedBaseFunction / (static_cast<double>(numberOfBasisFunctions - 1));
  for (int i = 0; i < (numberOfBasisFunctions + 2); i++) {
    mu = i * muSpacing - 2 * muSpacing;
    ...
    baseSecondDerivatives.push_back(-(1 / (sigma * sigma)) * getValueOfGaussian(time, mu, sigma) +
                                    pow((time - mu) / (sigma * sigma), 2) * getValueOfGaussian(time, mu, sigma));
  }
  //  return output;
}
...
void GaussianBaseTrajectory::setScalingOfBasisFunctions() {
  if (numberOfBasisfunctions_ != 1) {
    ...
    generateBasisFunctionVectors(baseFunctionVector, dummyVector, dummyVector2,
                                 timeBetweenFirstAndLastParametrizedBaseFunction_ / 2 -
                                     timeBetweenFirstAndLastParametrizedBaseFunction_ / (4.0 * (numberOfBasisfunctions_ - 1)),
...
GaussianBaseTrajectory::Parameters GaussianBaseTrajectory::elementWiseVectorMultiplication(const BaseValues& vecB,
                                                                                           const Parameters& vecP) const {
  Parameters output;
...
double GaussianBaseTrajectory::sum(const Parameters& parameters) const {
  double output = 0;
  for (double parameter : parameters) {
    output += parameter;
  }
