...
Line::Line(const VertexList& vertices, robot_utils::geometry::VertexOrder vertexOrder) : Polygon(vertices, vertexOrder) {
  assert(vertices_.size() == 2);
  lineCoefficients_ = Polygon::LineCoefficientList(1, {0.0, 0.0, 0.0});
}

Line::Line(VertexList&& vertices, robot_utils::geometry::VertexOrder vertexOrder) : Polygon(std::move(vertices), vertexOrder) {
  assert(vertices_.size() == 2);
  lineCoefficients_ = Polygon::LineCoefficientList(1, {0.0, 0.0, 0.0});
}
...
bool Line::findIntersection(const Line& lineA, const Line& lineB, Position2d& intersection) {
  ...
  if (x(0) >= 0.0 && x(0) <= 1.0) {
    intersection << lineA.getVertices()[0].x() + x(0) * v1(0), lineA.getVertices()[0].y() + x(0) * v1(1);
    return true;
  } else {
    return false;
  }
...
bool Line::isPointInPolygonInternal(const Position2d& point, double tol) const {
  // Test inequalities implied by the line coefficients for all sides of the polygon.
  const double a = lineCoefficients_.front()[0];
  const double b = lineCoefficients_.front()[1];
  const double c = lineCoefficients_.front()[2];
