...
Polygon::Polygon() : vertices_(), vertexOrder_(VertexOrder::Undefined), lineCoefficients_() {}

Polygon::Polygon(const VertexList& vertices, robot_utils::geometry::VertexOrder vertexOrder)
    : vertices_(vertices.begin(), vertices.end()), vertexOrder_(vertexOrder), lineCoefficients_() {}

Polygon::Polygon(VertexList&& vertices, robot_utils::geometry::VertexOrder vertexOrder) : vertexOrder_(vertexOrder), lineCoefficients_() {
  vertices_ = std::move(vertices);
}

Polygon::Polygon(const Position2d& vertexCenter, unsigned int numOfVertices, double polygonSize)
    : vertexOrder_(VertexOrder::CounterClockWise), lineCoefficients_() {
  ...
  RotationMatrix2d << cos(yawAngle), -sin(yawAngle), sin(yawAngle), cos(yawAngle);
...
Polygon::Polygon(const Position2d& vertexCenter, unsigned int numOfVertices, double polygonSize, const Eigen::Matrix2d& RotationMatrix2d)
    : vertexOrder_(VertexOrder::CounterClockWise), lineCoefficients_() {
  assert(numOfVertices >= 2u);
  vertices_.resize(numOfVertices);
...
Polygon::Polygon(const Polygon& polygon)
    : vertices_(polygon.getVertices()), vertexOrder_(polygon.getVertexOrder()), lineCoefficients_(polygon.getLineCoefficients()) {}
...
bool Polygon::reorderVertices(VertexOrder vertexOrder) {
  ...
  std::sort(vertices_.begin(), vertices_.end(), [&polygonCenter, &vertexOrder](const Position2d& p1, const Position2d& p2) -> bool {
    ...
    if (vertexOrder == VertexOrder::ClockWise) {
      ...
      std::cout << "[Polygon::reorderVertices] Undefined vertex order!\n";
      return false;
    }
  });
...
double Polygon::getPerimeter() const {
  ...
  for (unsigned int k = 0u; k < vertices_.size(); ++k) {
    perimeter += (vertices_[robot_utils::intmod(k + 1, vertices_.size())] - vertices_[k]).norm();
  }

  return perimeter;
}

void Polygon::setVertex(const Position2d& vertex, unsigned int vertexId) {
  assert(vertexId >= 0u && vertexId < vertices_.size());
  vertices_[vertexId] = vertex;
}
...
void Polygon::pushVertex(const Position2d& vertex) {
...
bool Polygon::updateLineCoefficients() {
  if (vertices_.size() <= 1u) {
    ...
    return getLineCoefficientsFromVertices(vertices_[0], vertices_[1], lineCoefficients_[0], vertexOrder_);
  }
  ...
  for (unsigned int vId = 0u; vId < vertices_.size(); ++vId) {
    ...
    BOOST_ASSERT_MSG(vIdNext < vertices_.size(),
                     ("vIdNext was: " + std::to_string(vIdNext) + " numVertices was: " + std::to_string(vertices_.size())).c_str());
    BOOST_ASSERT_MSG(vIdNext >= 0,
                     ("vIdNext was: " + std::to_string(vIdNext) + " numVertices was: " + std::to_string(vertices_.size())).c_str());
    BOOST_ASSERT_MSG(vId < vertices_.size(),
                     ("vId was: " + std::to_string(vId) + " numVertices was: " + std::to_string(vertices_.size())).c_str());
    BOOST_ASSERT_MSG(vId >= 0, ("vId was: " + std::to_string(vId) + " numVertices was: " + std::to_string(vertices_.size())).c_str());

    success &= getLineCoefficientsFromVertices(vertices_[vId], vertices_[vIdNext], lineCoefficients_[vId], vertexOrder_);
  }
...
bool Polygon::resizePolygonTowardsInCenter(double lengthLineToLine) {
  ...
  for (unsigned int k = 0u; k < vertices_.size(); ++k) {
    ...
    BOOST_ASSERT_MSG(kNext < vertices_.size(), ("k was: " + std::to_string(k) + " kNext was: " + std::to_string(kNext) +
                                                " numVertices was: " + std::to_string(vertices_.size()))
                                                   .c_str());
    BOOST_ASSERT_MSG(kNext >= 0, ("k was: " + std::to_string(k) + " kNext was: " + std::to_string(kNext) +
                                  " numVertices was: " + std::to_string(vertices_.size()))
                                     .c_str());
    BOOST_ASSERT_MSG(kPrev < vertices_.size(), ("k was: " + std::to_string(k) + " kPrev was: " + std::to_string(kPrev) +
                                                " numVertices was: " + std::to_string(vertices_.size()))
                                                   .c_str());
    BOOST_ASSERT_MSG(kPrev >= 0, ("k was: " + std::to_string(k) + " kPrev was: " + std::to_string(kPrev) +
                                  " numVertices was: " + std::to_string(vertices_.size()))
                                     .c_str());
...
bool Polygon::resizeAreaTriangleBased(double percent) {
  ...
  for (unsigned int id = 0u; id < vertices_.size(); ++id) {
    averageIncircleRadius += computeInRadiusOfTriangle(vertices_[id], vertices_[robot_utils::intmod(id + 1u, vertices_.size())],
...
bool Polygon::isPointInPolygon(const Position2d& point, double tol) const {
  return isPointInPolygonInternal(point, tol);
}
...
void Polygon::projectPolygonToRange(const Polygon& polygon, const Position2d& axisDirection, Eigen::Vector2d& range) {
  // Initialization.
...
bool Polygon::doRangesOverlap(const Position2d& rangeA, const Position2d& rangeB) {
...
bool Polygon::isPointInPolygonInternal(const Position2d& point, double tol) const {
  if (vertices_.empty()) {
    return false;
  ...
  for (const auto& lineCoefficient : lineCoefficients_) {
    const double projection = lineCoefficient[0] * point.x() + lineCoefficient[1] * point.y();
    ...
    if (vertexOrder_ == robot_utils::geometry::VertexOrder::CounterClockWise) {
      ...
      //      ax + by >= -c.
      if (!(projection >= -lineCoefficient[2] - tol)) {
        return false;
      }
