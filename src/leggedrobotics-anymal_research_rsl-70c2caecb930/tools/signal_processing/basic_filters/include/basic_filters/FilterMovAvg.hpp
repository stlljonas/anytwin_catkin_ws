...
class FilterMovAvg {
 public:
  FilterMovAvg() : FilterMovAvg(1) {}

  /*!
   * @param nSamples	number of samples of the window
   */
  explicit FilterMovAvg(const unsigned int nSamples, const ValueType_& initValue = getDefaultValue<ValueType_>()) {
   ...
   */
  inline ValueType_ getFilteredValue() const { return (sum_ / static_cast<double>(samples_.size())); }
