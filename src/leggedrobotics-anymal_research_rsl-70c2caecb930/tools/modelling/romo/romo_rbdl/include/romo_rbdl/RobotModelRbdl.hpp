...
class RobotModelRbdl : public romo::RobotModel<ConcreteDescription_, RobotState_> {
 ...
 public:
  ...
  bool initializeFromUrdf(const std::string& urdfString, bool verbose = false) {
    return initializeFromUrdfImpl(urdfString, verbose);
  }

  const std::string & getUrdfDescription() { return urdfDescription_; }
 ...
 ******************/
  ...
  unsigned int getBranchMoveableLinksCount(const BranchEnum& branchEnum) override;

  Eigen::VectorXd getInertialParameters() override;

  Eigen::VectorXd getBodyInertialParameters(const BodyEnum& bodyEnum) override;

  Eigen::VectorXd getBranchInertialParameters(BranchEnum branchEnum) override;

  Eigen::MatrixXd getBranchRegressor(const BranchEnum& branchEnum, bool verbose) override;

  Eigen::MatrixXd getIndividualBodyRegressor(const BodyEnum& bodyEnum, bool verbose) override;
 ...
 protected:
  ...
  virtual bool initializeFromUrdfImpl(const std::string &urdfString, bool verbose);

  template <typename CD_ = ConcreteDescription_, typename RS_ = RobotState_>
  Eigen::MatrixXd getIndividualBodyRegressorImpl(const BodyEnum& bodyEnum, bool verbose, typename std::enable_if<std::is_base_of<romo::ExtendedRobotState<CD_>, RS_>::value>::type* = nullptr);
  ...
  //! Urdf description represented as a string
...
#include <romo_rbdl/RobotModelRbdl_implementation_identification.tpp>
