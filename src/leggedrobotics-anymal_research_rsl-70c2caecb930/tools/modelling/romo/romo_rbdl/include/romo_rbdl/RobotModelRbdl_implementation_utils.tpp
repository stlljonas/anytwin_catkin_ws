...
template<typename ConcreteDescription_, typename RobotState_>
bool RobotModelRbdl<ConcreteDescription_, RobotState_>::initializeFromUrdfImpl(const std::string& urdfString, bool verbose) {
  ...
  if( urdfString.size() >= suffix.size() && urdfString.compare(urdfString.size() - suffix.size(), suffix.size(), suffix) == 0 ) {
    ...
    urdfDescription_ = romo::URDFStringFromFile(urdfString.c_str());
    if( urdfString.empty() ) {
      MELO_ERROR_STREAM("[RobotModel::initializeFromUrdf] Error parsing URDF from file.");
      return false;
    }
  } else {
    urdfDescription_ = urdfString;
  }

  // Initialize romo from a urdf string
  if( !romo::URDFReadFromString(urdfModel, urdfDescription_.c_str(), rbdlModel_.get(), joints, verbose) ) {
    MELO_ERROR_STREAM("[RobotModel::initializeFromUrdf] Error parsing URDF from string.");
    return false;
  }
  MELO_DEBUG_STREAM("[RobotModel:initializeFromUrdf]: Parsed URDF for romo from string!");
