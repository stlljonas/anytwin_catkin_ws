...
bool construct_model(Model* rbdl_model, ModelPtr urdf_model,
  ...
  if(urdf_model == nullptr) {
    MELO_INFO_STREAM("[romo::construct_model] URDF model is null.");
    return false;
  }
...
std::string URDFStringFromFile(const char* filename) {

  std::ifstream model_file (filename);
  if (!model_file) {
    MELO_WARN_STREAM("[romo::URDFStringFromFile] Error opening file: " << filename);
    return std::string{};
  }
...
bool URDFReadFromFile(urdf::ModelInterfaceSharedPtr& urdf_model,
  ...
  std::string model_xml_string = URDFStringFromFile(filename);
  if(model_xml_string.empty()) {
    return false;
  }
...
bool URDFReadFromString(urdf::ModelInterfaceSharedPtr& urdf_model,
  ...
  if(urdf_model == nullptr) {
    MELO_INFO_STREAM("[romo::URDFReadFromFile] URDF model could not be parsed.");
    return false;
  }
