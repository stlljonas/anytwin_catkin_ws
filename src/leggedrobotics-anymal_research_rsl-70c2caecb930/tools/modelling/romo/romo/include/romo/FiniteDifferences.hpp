...
static void estimateLinearVelocityBody(
  ...
  stateCopy = state;
  for (int i=0; i<stateCopy.getNumberOfGeneralizedVelocities(); ++i) { stateCopy = stateCopy.boxPlus(qDt_plus(i), i, true); }
  robotModel.setState(stateCopy, true, true, false);
  ...
  robotModel.setState(state, true, true, false);
}
...
static void estimateTranslationHessianWorldToPointOnBodyForState(
  ...
  robotModel.setState(state.boxPlus(internal::deltaH, perturbedStateIndex, true), true, true, false);
...
static void estimateHessianWorldToComInWorldFrameForState(
  ...
  stateTemp = state.boxPlus(internal::deltaH, perturbedStateIndex, true);
  robotModel.setState(stateTemp, true, true, false);
...
static void estimateTranslationalJacobianWorldToBody(
  ...
  for (int i=0; i<state.getNumberOfGeneralizedVelocities(); ++i) {
    ...
    robotModel.setState(state.boxPlus(internal::deltaH, i, true), true, true, false);
...
static void estimateTranslationalJacobianWorldToBodyComInWorldFrame(
  ...
  for (int i=0; i<state.getNumberOfGeneralizedVelocities(); ++i) {
    // Positive perturbation to i-th entry of the configuration vector.
    stateTemp = state.boxPlus(internal::deltaH, i, true);
    robotModel.setState(stateTemp, true, true, false);
...
static void estimateTranslationalJacobianTimeDerivativeWorldToWholeBodyComTimeBased(
  ...
  for (int i=0; i<stateCopy.getNumberOfGeneralizedVelocities(); ++i) {
    stateCopy = stateCopy.boxPlus(qDt_plus(i), i, true);
  }
  robotModel.setState(stateCopy, true, true, false);
...
static void estimateTranslationalJacobianTimeDerivativeWorldToPointOnBodyTimeBased(
  ...
  stateCopy = state;
  for (auto i=0; i<stateCopy.getNumberOfGeneralizedVelocities(); ++i) { stateCopy = stateCopy.boxPlus(qDt_plus(i), i, true); }
  robotModel.setState(stateCopy, true, true, false);
  Eigen::MatrixXd dj_p = Eigen::MatrixXd::Zero(3, nU);
...
static void estimateChristoffelSymbol(
  ...
  robotModel.setState(state.boxPlus(internal::deltaH, k, true), true, true, false);
  Bp.setZero();
  ...
  robotModel.setState(state.boxPlus(internal::deltaH, j, true), true, true, false);
  Bp.setZero();
  robotModel.getMassInertiaMatrix(Bp);
