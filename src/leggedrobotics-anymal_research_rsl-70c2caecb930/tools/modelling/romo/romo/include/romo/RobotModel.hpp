 ...
 ********************/
  ...
  /*! Gets the number of moveable links on a branch.
  * @param branchEnum               the branch enum for which the moveable links should be computed
  * @returns                        the number of moveable links
  */

  virtual unsigned int getBranchMoveableLinksCount(const BranchEnum& branchEnum) = 0;

  /*! Gets the inertial parameters of the whole robot.
   * For each robot link, 10 parameters are collected (mass, first moment of mass, and inertia tensor).
   * The first moment of mass as well as the inertia tensor are expressed in the body-fixed joint frame.
   * The inertia tensor is expressed around the joint frame using Huygens-Steiner theorem.
   * The parameters returned are as follows: inertialParameters = [m_0, lx_0, ly_0, lz_0, Ixx_0, Ixy_0, Ixz_0, Iyy_0, Iyz_0, Izz_0, ...]
   * where l is the first moment of mass in the body frame, and I is the inertia tensor around the body joint frame and the index, e.g. 0
   * denotes the link index.
   * @returns                        inertialParameters
   */

  virtual Eigen::VectorXd getInertialParameters() = 0;

  /*! Gets the inertial parameters of an individual body.
   * See getInertialParameters doc. for the semantics of the returned inertial parameters.
   * Gets the inertial parameters for all the moveable links of the branch.
   * @returns                            bodyInertialParameters
   */

  virtual Eigen::VectorXd getBodyInertialParameters(const BodyEnum& bodyEnum) = 0;

  /*! Gets the inertial parameters of an individual robot branch.
   * See getInertialParameters doc. for the semantics of the returned inertial parameters.
   * Gets the inertial parameters for all the moveable links of the branch.
   * @param branchEnum                     the branch enum for which the inertial parameters should be computed
   * @returns                              branchInertialParameters
   */

  virtual Eigen::VectorXd getBranchInertialParameters(BranchEnum branchEnum) = 0;

  /*! Gets the regressor of a given branch assuming that the base link is fixed. The regressor is computed for the current robot state.
   * Getting the regressor of the base branch is not supported by this function.
   * @param branchEnum                the branch enum for which the regressor should be computed
   * @param verbose                   prints debug output if true
   * @returns                         the branchRegressor
   */

  virtual Eigen::MatrixXd getBranchRegressor(const BranchEnum& branchEnum, bool verbose) = 0;

  /*! Gets the regressor of an individual body assuming that the base link is fixed. The regressor is computed for the current robot state.
   * Getting the regressor of the base branch is not supported by this function.
   * @param bodyEnum                          the body enum for which the regressor should be computed
   * @param verbose                           prints debug output if true
   * @returns                                 individualBodyRegressor
   */

  virtual Eigen::MatrixXd getIndividualBodyRegressor(const BodyEnum& bodyEnum, bool verbose) = 0;
      ...
      [](JointPositions& positions, const Eigen::VectorXd& jointUpdate, const LimbEnum limbEnum)
        {
          ...
          positions.toImplementation().segment(limbStartId, numDofInvKin) += jointUpdate;
        }
    );
