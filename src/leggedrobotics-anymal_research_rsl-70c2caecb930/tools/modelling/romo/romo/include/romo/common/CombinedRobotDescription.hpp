...
struct CombinedDefinitions {
 ...
 public:
  ...
  inline static constexpr auto getNumDofLimbImpl(LimbEnum limb)
  -> typename std::enable_if<std::is_class<typename CombinedTopology::mapLimbToNDof>::value, RType>::type
  {  return CombinedTopology::mapLimbToNDof::at(limb); }

  /**
   * Return the WheelDiameter for a limb, under the assumption that the first of the combined RDs contains the wheel limb.
   */
  BOOST_TTI_HAS_STATIC_MEMBER_FUNCTION(getWheelDiameterImpl);
  using HasGetWheelDiameterImpl = has_static_member_function_getWheelDiameterImpl<ConcreteDefinitions1, double ()>;
  template<typename RType = double>
  inline static constexpr auto getWheelDiameterImpl(LimbEnum limb)
  -> typename std::enable_if<HasGetWheelDiameterImpl::value, RType>::type
  { return ConcreteDefinitions1::getWheelDiameterImpl(); /* issue: Not passing LimbEnum */ }

  //! @return index of the endeffector in the limb joints list
  template<typename RType = unsigned int>
  inline static constexpr auto getWheelIndexInJImpl(LimbEnum limb)
  -> typename std::enable_if<std::is_class<typename CombinedTopology::mapLimbToWheelIndexInJ>::value, RType>::type
