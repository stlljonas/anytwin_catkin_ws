...
private:
 ...
 TestRigidBodyBase(const std::string& name,
  ...
  const kindr::HomTransformMatrixD& getTransform() const{
    return transform_;
  }
...
template <typename int_convertible1, typename int_convertible2 >
std::ostream& operator<<(std::ostream& os, const std::pair<int_convertible1, int_convertible2>& obj)
{
      os << "(" << (int)obj.first <<", "<<(int)obj.second <<")";
      return os;
}

// Helper function that checks if a vector contains a specific item.
// This won't work if the item doesn't have an ostream overload.
template <typename item>
void expectContains(std::vector<item> v, item element){
  std::stringstream message;
  message << "Vector contains ";
  for (item cur_item: v){
    message << cur_item;
  }
  message <<"; desired element is "<<element;
  EXPECT_TRUE(std::find(v.begin(), v.end(), element) != v.end())<< message.str();
}
} // namespace collisions_fcl_tests
