...
TEST_F(DynamicsFixture, TestCalcDynamicSingleChain) {
	...
	for (i = 0; i < QDDot.size(); i++) {
		ANY_RBDL_LOG << "QDDot[" << i << "] = " << QDDot[i] << endl;
	}

	for (i = 0; i < model->a.size(); i++) {
		ANY_RBDL_LOG << "a[" << i << "]     = " << model->a[i] << endl;
	}
...
TEST ( Dynamics, TestForwardDynamicsLagrangian) {
	Model model;
	...
	KINDR_ASSERT_DOUBLE_MX_EQ_ABS_REL (QDDot_aba, QDDot_lagrangian, TEST_PREC, TEST_PREC, "");
}
 ...
 * it here.
 */
TEST ( Dynamics, TestForwardDynamics3DoFModel) {
	Model model;
...
//	cout << LogOutput.str() << endl;
	...
	KINDR_ASSERT_DOUBLE_MX_EQ_ABS_REL (QDDot_ref, QDDot, TEST_PREC, TEST_PREC, "");
}
 ...
 */
TEST ( Dynamics, TestForwardDynamics3DoFModelLagrangian) {
	Model model;
...
//	cout << LogOutput.str() << endl;
	...
	KINDR_ASSERT_DOUBLE_MX_EQ_ABS_REL (QDDot_ab, QDDot_lagrangian, TEST_PREC, TEST_PREC, "");
}
 ...
 */
TEST ( Dynamics, TestForwardDynamicsTwoLegModelLagrangian) {
	Model *model = NULL;
	...
	ANY_RBDL_LOG << "--- model created (" << model->dof_count << " DOF) ---" << endl;
...
//	cout << LogOutput.str() << endl;
	...
	KINDR_ASSERT_DOUBLE_MX_EQ_ABS_REL (QDDotABA, QDDot, TEST_PREC, TEST_PREC, "");
...
TEST_F(FixedAndMovableJoint, TestForwardDynamicsFixedJoint) {
	...
	KINDR_ASSERT_DOUBLE_MX_EQ_ABS_REL (QDDot_fixed_emulate, QDDot_fixed, TEST_PREC, TEST_PREC, "");
}
...
TEST_F(FixedAndMovableJoint, TestInverseDynamicsFixedJoint) {
	...
	KINDR_ASSERT_DOUBLE_MX_EQ_ABS_REL (Tau_2dof, Tau_fixed, TEST_PREC, TEST_PREC, "");
}
...
TEST_F ( FloatingBase12DoF, TestForwardDynamicsLagrangianPrealloc ) {
	...
	KINDR_ASSERT_DOUBLE_MX_EQ_ABS_REL (QDDot, QDDot_prealloc, 1e-14, 1e-14,  "");
}
