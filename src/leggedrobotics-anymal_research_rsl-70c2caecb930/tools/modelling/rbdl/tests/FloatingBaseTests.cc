...
struct FloatingBaseFixture : ::testing::Test {
	FloatingBaseFixture () {
		ClearLogOutput();
		model = new Model;
...
TEST_F ( FloatingBaseFixture, TestCalcPointTransformation ) {
	...
	KINDR_ASSERT_DOUBLE_MX_EQ_ABS_REL (Vector3d (0., -1., 0.), test_point, TEST_PREC, TEST_PREC, "");
}
...
TEST_F(FloatingBaseFixture, TestCalcDynamicFloatingBaseDoubleImplicit) {
	...
	for (i = 0; i < model->a.size(); i++) {
		ANY_RBDL_LOG << "a[" << i << "]     = " << model->a.at(i) << endl;
	}
...
TEST_F(FloatingBaseFixture, TestCalcPointVelocityCustom) {
	...
	KINDR_ASSERT_DOUBLE_MX_EQ_ABS_REL (point_base_velocity_reference, point_base_velocity, TEST_PREC, TEST_PREC, "");
}
...
//	cout << "world_accel = " << point_world_acceleration << endl;
	...
	KINDR_ASSERT_DOUBLE_MX_EQ_ABS_REL (humans_point_position, point_world_position, TEST_PREC, TEST_PREC, "");
	KINDR_ASSERT_DOUBLE_MX_EQ_ABS_REL (humans_point_velocity, point_world_velocity, TEST_PREC, TEST_PREC, "");
	KINDR_ASSERT_DOUBLE_MX_EQ_ABS_REL (humans_point_acceleration, point_world_acceleration, TEST_PREC, TEST_PREC, "");
}
