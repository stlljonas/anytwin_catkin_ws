...
struct ANY_RBDL_DLLAPI Body {
	...
	void Join (const Math::SpatialTransform &transform, const Body &other_body) __attribute__ ((deprecated)) {
		...
		ANY_RBDL_LOG << "other_com = " << std::endl << other_com.transpose() << std::endl;
		ANY_RBDL_LOG << "rotation = " << std::endl << transform.E << std::endl;
		...
		ANY_RBDL_LOG << "inertia_other = " << std::endl << inertia_other << std::endl;
		...
		ANY_RBDL_LOG << "inertia_other_com = " << std::endl << inertia_other_com << std::endl;

		// 2. Rotate the inertia that it is aligned to the frame of this body
		Math::Matrix3d inertia_other_com_rotated = transform.E.transpose() * inertia_other_com * transform.E;
		ANY_RBDL_LOG << "inertia_other_com_rotated = " << std::endl << inertia_other_com_rotated << std::endl;
		...
		ANY_RBDL_LOG << "inertia_other_com_rotated_this_origin = " << std::endl << inertia_other_com_rotated_this_origin << std::endl;

		// 4. Sum the two inertias
		Math::Matrix3d inertia_summed = Math::Matrix3d (this_rbi.toMatrix().block<3,3>(0,0)) + inertia_other_com_rotated_this_origin;
		ANY_RBDL_LOG << "inertia_summed  = " << std::endl << inertia_summed << std::endl;
		...
		ANY_RBDL_LOG << "new_mass = " << new_mass << std::endl;
		ANY_RBDL_LOG << "new_com  = " << new_com.transpose() << std::endl;
		ANY_RBDL_LOG << "new_inertia  = " << std::endl << new_inertia << std::endl;
