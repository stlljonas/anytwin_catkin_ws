...
#ifndef ANY_RBDL_ENABLE_LOGGING
	#define ANY_RBDL_LOG if (false) LogOutput
	#define SUPPRESS_LOGGING ;
#else
	#define ANY_RBDL_LOG LogOutput
	#define SUPPRESS_LOGGING LoggingGuard _nolog
#endif

extern ANY_RBDL_DLLAPI std::ostringstream LogOutput;
ANY_RBDL_DLLAPI void ClearLogOutput ();
 ...
 *
 */
class ANY_RBDL_DLLAPI LoggingGuard {
	public:
		LoggingGuard() {
			log_backup.str("");
