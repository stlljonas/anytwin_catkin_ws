...
void ForwardDynamics (
	...
	ANY_RBDL_LOG << "Q          = " << Q.transpose() << std::endl;
	ANY_RBDL_LOG << "QDot       = " << QDot.transpose() << std::endl;
	ANY_RBDL_LOG << "Tau        = " << Tau.transpose() << std::endl;
	...
	for (i = 1; i < model.mBodies.size(); i++) {
		...
		/*
		ANY_RBDL_LOG << "X_J (" << i << "):" << std::endl << X_J << std::endl;
		ANY_RBDL_LOG << "v_J (" << i << "):" << std::endl << v_J << std::endl;
		ANY_RBDL_LOG << "v_lambda" << i << ":" << std::endl << model.v.at(lambda) << std::endl;
		ANY_RBDL_LOG << "X_base (" << i << "):" << std::endl << model.X_base[i] << std::endl;
		ANY_RBDL_LOG << "X_lambda (" << i << "):" << std::endl << model.X_lambda[i] << std::endl;
		ANY_RBDL_LOG << "SpatialVelocity (" << i << "): " << model.v[i] << std::endl;
		*/
		...
		if (f_ext != NULL && (*f_ext)[i] != SpatialVectorZero) {
			ANY_RBDL_LOG << "External force (" << i << ") = " << model.X_base[i].toMatrixAdjoint() * (*f_ext)[i] << std::endl;
...
//			ANY_RBDL_LOG << "multdof3_u[" << i << "] = " << model.multdof3_u[i].transpose() << std::endl;
			unsigned int lambda = model.lambda[i];
			if (lambda != 0) {
...
//			ANY_RBDL_LOG << "u[" << i << "] = " << model.u[i] << std::endl;
...
void ForwardDynamicsLagrangian (
	...
	ANY_RBDL_LOG << "A = " << std::endl << *H << std::endl;
	ANY_RBDL_LOG << "b = " << std::endl << *C * -1. + Tau << std::endl;
...
//			std::cout << "i: " << dof_index_i << " Mii = " << Hs << std::endl;
			...
			while (model.lambda[j] != 0) {
				...
				if (model.mJoints[j].mDoFCount == 3) {
					...
					ANY_RBDL_LOG << F.transpose() << std::endl << model.multdof3_S[j] << std::endl;
					ANY_RBDL_LOG << H_temp2.transpose() << std::endl;
