...
void SolveContactSystemDirect (
	...
	ANY_RBDL_LOG << "A = " << std::endl << A << std::endl;
	ANY_RBDL_LOG << "b = " << std::endl << b << std::endl;
	...
	switch (linear_solver) {
		...
		case (LinearSolverHouseholderQR) :
			...
			break;
		default:
			ANY_RBDL_LOG << "Error: Invalid linear solver: " << linear_solver << std::endl;
			assert (0);
			break;
	}

	ANY_RBDL_LOG << "x = " << std::endl << x << std::endl;
}
...
void ForwardDynamicsApplyConstraintForces (
	...
	for (i = 1; i < model.mBodies.size(); i++) {
		...
		if (CS.f_ext_constraints[i] != SpatialVectorZero) {
			ANY_RBDL_LOG << "External force (" << i << ") = " << model.X_base[i].toMatrixAdjoint() * CS.f_ext_constraints[i] << std::endl;
...
// ClearLogOutput();
	...
	ANY_RBDL_LOG << "--- first loop ---" << std::endl;
...
#endif
				ANY_RBDL_LOG << "pA[" << lambda << "] = " << model.pA[lambda].transpose() << std::endl;
			}
		} else {
	...
	ANY_RBDL_LOG << "QDDot = " << QDDot.transpose() << std::endl;
}
...
void ForwardDynamicsAccelerationDeltas (
	...
	for (unsigned int i = 0; i < f_t.size(); i++) {
		ANY_RBDL_LOG << "f_t[" << i << "] = " << f_t[i].transpose() << std::endl;
	}

	for (unsigned int i = 0; i < model.mBodies.size(); i++) {
		ANY_RBDL_LOG << "i = " << i << ": d_pA[i] " << CS.d_pA[i].transpose() << std::endl;
	}
	for (unsigned int i = 0; i < model.mBodies.size(); i++) {
		ANY_RBDL_LOG << "i = " << i << ": d_u[i] = " << CS.d_u[i] << std::endl;
	}
	...
	for (unsigned int i = 1; i < model.mBodies.size(); i++) {
		...
		ANY_RBDL_LOG << "QDDot_t[" << i - 1 << "] = " << QDDot_t[i - 1] << std::endl;
		ANY_RBDL_LOG << "d_a[i] = " << CS.d_a[i].transpose() << std::endl;
	}
}
...
void ForwardDynamicsContactsKokkevis (
	...
	ANY_RBDL_LOG << "=== Initial Loop Start ===" << std::endl;
	...
	for (ci = 0; ci < CS.size(); ci++) {
		...
		ANY_RBDL_LOG << "body_id = " << body_id << std::endl;
		ANY_RBDL_LOG << "point = " << point << std::endl;
		ANY_RBDL_LOG << "normal = " << normal << std::endl;
		ANY_RBDL_LOG << "QDDot_0 = " << CS.QDDot_0.transpose() << std::endl;
		{
			SUPPRESS_LOGGING;
			UpdateKinematicsCustom (model, NULL, NULL, &CS.QDDot_0);
	...
	for (ci = 0; ci < CS.size(); ci++) {
		...
		ANY_RBDL_LOG << "normal = " << normal.transpose() << std::endl;

		Vector3d point_global = CalcBodyToBaseCoordinates (model, Q, body_id, point, false);
		ANY_RBDL_LOG << "point_global = " << point_global.transpose() << std::endl;
		...
		ANY_RBDL_LOG << "f_t[" << movable_body_id << "] = " << CS.f_t[ci].transpose() << std::endl;

		{
//			SUPPRESS_LOGGING;
			ForwardDynamicsAccelerationDeltas (model, CS, CS.QDDot_t, movable_body_id, CS.f_ext_constraints);
			ANY_RBDL_LOG << "QDDot_0 = " << CS.QDDot_0.transpose() << std::endl;
			ANY_RBDL_LOG << "QDDot_t = " << (CS.QDDot_t + CS.QDDot_0).transpose() << std::endl;
			ANY_RBDL_LOG << "QDDot_t - QDDot_0= " << (CS.QDDot_t).transpose() << std::endl;
		}
		...
		for (unsigned int cj = 0; cj < CS.size(); cj++) {
			...
			ANY_RBDL_LOG << "point_accel_0  = " << CS.point_accel_0[ci].transpose() << std::endl;
			CS.K(ci,cj) = CS.normal[cj].dot(point_accel_t - CS.point_accel_0[cj]);
			ANY_RBDL_LOG << "point_accel_t = " << point_accel_t.transpose() << std::endl;
		}
	}

	ANY_RBDL_LOG << "K = " << std::endl << CS.K << std::endl;
	ANY_RBDL_LOG << "a = " << std::endl << CS.a << std::endl;
	...
	switch (CS.linear_solver) {
		...
		case (LinearSolverHouseholderQR) :
			...
			break;
		default:
			ANY_RBDL_LOG << "Error: Invalid linear solver: " << CS.linear_solver << std::endl;
			assert (0);
			break;
	}
#else
	...
	ANY_RBDL_LOG << "f = " << CS.force.transpose() << std::endl;
	...
	for (ci = 0; ci < CS.size(); ci++) {
		...
		ANY_RBDL_LOG << "f_ext[" << movable_body_id << "] = " << CS.f_ext_constraints[movable_body_id].transpose() << std::endl;
	}
	...
	ANY_RBDL_LOG << "QDDot after applying f_ext: " << QDDot.transpose() << std::endl;
}
