...
bool EthercatBusBase::startup(const bool sizeCheck) {
  ...
  globalWorkingCounterTooLowCounter_ = 0;
  initlialized_ = true;
...
void EthercatBusBase::updateRead() {
  ...
  if (!workingCounterIsOk()) {
    ...
    ++globalWorkingCounterTooLowCounter_;
    if (!busIsOk()) {
...
unsigned int EthercatBusBase::getGlobalWorkingCounterTooLowCount() const { return globalWorkingCounterTooLowCounter_; }
