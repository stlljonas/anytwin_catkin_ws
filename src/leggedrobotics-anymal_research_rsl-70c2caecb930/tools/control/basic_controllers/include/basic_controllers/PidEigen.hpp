...
class PidEigen {
 public:
   ...
   */
  PidEigen(const MatrixXd& pGains, const MatrixXd& iGains, const MatrixXd& dGains, const MatrixXd& iMax, const MatrixXd& iMin,
           const MatrixXd& cmdMax, const MatrixXd& cmdMin);

  virtual ~PidEigen() = default;
   ...
   */
  void init(const MatrixXd& pGains, const MatrixXd& iGains, const MatrixXd& dGains, const MatrixXd& iMax, const MatrixXd& iMin,
            const MatrixXd& cmdMax, const MatrixXd& cmdMin);
  ...
  void update(const Eigen::MatrixBase<Derived>& error, Eigen::MatrixBase<Derived>& command, double dt) {
    ...
    BOOST_ASSERT_MSG(error.rows() == command.rows(),
                     ("'error' rows: " + std::to_string(error.rows()) + " command rows: " + std::to_string(command.rows())).c_str());
    BOOST_ASSERT_MSG(error.cols() == command.cols(),
                     ("'error' cols: " + std::to_string(error.cols()) + " command cols: " + std::to_string(command.cols())).c_str());
