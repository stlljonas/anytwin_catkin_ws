...
class ChronoTimer {
 public:
  ...
  explicit ChronoTimer(std::string timerNamespace = "") : alpha_(0.01), timerNamespace_(std::move(timerNamespace)) {
   ...
   */
  void pinTime(const std::string& timerName = "default") { std::get<1>(timerMap_[timerName]) = clock::now(); }
  ...
  std::string asString() {
    ...
    for (const auto& timer : timerMap_) {
      stream << "Average elapsed time for timer '" << std::get<0>(timer) << "' is " << getAverageElapsedTimeUSec(std::get<0>(timer))
             << "us. " << std::endl;
    }
    stream << "*********" << std::endl;
    return stream.str();
...
using SteadyClockTimer = internal::ChronoTimer<std::chrono::steady_clock>;
using SystemClockTimer = internal::ChronoTimer<std::chrono::system_clock>;
using HighResolutionClockTimer = internal::ChronoTimer<std::chrono::high_resolution_clock>;
