...
template <typename key_type_, template <typename...> class container_ = std::map>
struct wrapper_map {
...
template <template <typename...> class container_ = std::vector>
struct wrapper_vec {
  //
  template <typename iterator_, typename element_type_>
 ...
 */
template <typename element_type_, typename container_wrapper_, template <typename...> class smart_pointer_ = std::unique_ptr>
struct smart_pointer_container {
  ...
  iterator s_begin() { return items_.begin(); }              // NOLINT(readability-identifier-naming)
  iterator s_end() { return items_.end(); }                  // NOLINT(readability-identifier-naming)
  const_iterator s_begin() const { return items_.begin(); }  // NOLINT(readability-identifier-naming)
  const_iterator s_end() const { return items_.end(); }      // NOLINT(readability-identifier-naming)

  size_type size() const { return items_.size(); }
  void clear() { items_.clear(); }

  // NOLINTNEXTLINE(readability-identifier-naming)
  observer_ptr_container make_observer_container() const { return observer_ptr_container(this->s_begin(), this->s_end()); }
...
template <typename element_type_, template <typename...> class smart_pointer_ = std::unique_ptr,
          template <typename...> class container_ = std::vector>
...
template <typename key_type_, typename element_type_, template <typename...> class smart_pointer_ = std::unique_ptr,
          template <typename...> class container_ = std::map>
