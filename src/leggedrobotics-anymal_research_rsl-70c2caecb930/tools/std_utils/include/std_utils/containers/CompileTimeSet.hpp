...
class CompileTimeSet<Type_> {
  ...
  */
  ...
  /*! \brief Default implementation of contains().
      ...
      \return false
  */
  static constexpr bool contains(const Type_ /*type*/) { return false; }
  ...
  */
  ...
  /*! \brief Implementation helper for std::function based forEachRange.
      ...
      \return false.
  */
  static constexpr bool forEachRangePreBegin(Type_ /*begin*/, Type_ /*end*/, std::function<bool(const Type_&)> /*f*/) { return false; }
  ...
  */
  ...
  /*! \brief Implementation helper for std::function based forEachRange.
      ...
      \return false.
  */
  static constexpr bool forEachRangePostBegin(Type_ /*begin*/, Type_ /*end*/, std::function<bool(const Type_&)> /*f*/) { return false; }
  ...
  */
  ...
  /*! \brief Default implementation of the unique().
      ...
      \return true.
  */
  static constexpr bool unique(const Type_ /*type*/) { return true; }
};
...
class CompileTimeSet<Type_, Value_, REST_...> {
  ...
  */
  ...
  /*! \brief Implementation of forEach: Call f() on every type until f returns false.
      ...
      \return true, iff all f's returned true.
   */
  static constexpr bool forEach(std::function<bool(const Type_&)> f) { return f(Value_) && CompileTimeSet<Type_, REST_...>::forEach(f); }
  ...
  */
  ...
  static constexpr bool forEachRange(Type_ begin, Type_ end, std::function<bool(const Type_&)> f) {
    return containsRange(begin, end) &&
           (Value_ == begin ? f(Value_) && ((begin == end) ? true : CompileTimeSet<Type_, REST_...>::forEachRangePostBegin(begin, end, f))
  ...
  */
  static constexpr bool contains(const Type_ t) { return (t == Value_) ? true : CompileTimeSet<Type_, REST_...>::contains(t); }
  ...
  */
  ...
  static constexpr bool forEachRangePreBegin(Type_ begin, Type_ end, Args&&... args) {
    return (
        Value_ == begin
            ? F<Value_>()(args...) &&
                  ((begin == end) ? true : CompileTimeSet<Type_, REST_...>::template forEachRangePostBegin<F, Args...>(begin, end, args...))
            : CompileTimeSet<Type_, REST_...>::template forEachRangePreBegin<F, Args...>(begin, end, args...));
  }
  ...
  */
  static constexpr bool forEachRangePreBegin(Type_ begin, Type_ end, std::function<bool(const Type_&)> f) {
    return (Value_ == begin ? f(Value_) && ((begin == end) ? true : CompileTimeSet<Type_, REST_...>::forEachRangePostBegin(begin, end, f))
                            : CompileTimeSet<Type_, REST_...>::forEachRangePreBegin(begin, end, f));
  }
  ...
  */
  ...
  static constexpr bool forEachRangePostBegin(Type_ begin, Type_ end, Args&&... args) {
    return Value_ == end
               ? F<Value_>()(args...)
               : F<Value_>()(args...) && CompileTimeSet<Type_, REST_...>::template forEachRangePostBegin<F, Args...>(begin, end, args...);
  }
  ...
  */
  static constexpr bool forEachRangePostBegin(Type_ begin, Type_ end, std::function<bool(const Type_&)> f) {
    return Value_ == end ? f(Value_) : f(Value_) && CompileTimeSet<Type_, REST_...>::forEachRangePostBegin(begin, end, f);
  }
  ...
  */
  static constexpr bool unique(const Type_ t) { return (t == Value_) ? false : CompileTimeSet<Type_, REST_...>::unique(t); }
};
...
struct cts_erase<Type_, CompileTimeSet<Type_, t, Values_...>, erase_type> {
  using type = typename std::conditional<
      ...
      cts_insert_front_t<Type_, typename cts_erase<Type_, CompileTimeSet<Type_, Values_...>, erase_type>::type, t>>::type;
};
...
struct cts_insert_front_if {
  ...
  //! Improve compiler output
  static_assert(!std::is_same<Type_, Type_>::value, "[cts_insert_front_if] Could not front-insert value conditionally into set!");
};
...
struct cts_insert_front_if<Type_, CompileTimeSet<Type_, Values_...>, value, Condition_> {
  using type = typename std::conditional<Condition_, CompileTimeSet<Type_, value, Values_...>, CompileTimeSet<Type_, Values_...>>::type;
};
