...
class CompileTimeMap<KeyType_, ValueType_, REST_...> {
  ...
  */
  ...
  /*! \brief Default implementation of contains().
      ...
      \return false.
  */
  static constexpr bool contains(const KeyType_ /*key*/) { return false; }
  ...
  /*! \brief Default implementation of containsValue().
      ...
      \return false.
  */
  static constexpr bool containsValue(const ValueType_ /*value*/) { return false; }
  ...
  */
  static constexpr ValueType_ atRecursive(const KeyType_ key) {
    return contains(key) ? ValueType_()
  ...
  */
  static constexpr KeyType_ findRecursive(const ValueType_ value) {
    return containsValue(value) ? KeyType_()
  ...
  /*! \brief Default implementation of the uniqueKey().
      ...
      \return true.
  */
  static constexpr bool uniqueKey(const KeyType_ /*key*/) { return true; }

  /*! \brief Implementation of the uniqueValue().
      \param value value that is checked for uniqueness
      \return true.
  */
  static constexpr bool uniqueValue(const ValueType_ /*value*/) { return true; }
};
...
class CompileTimeMap<KeyType_, ValueType_, KeyValuePair<KeyType_, ValueType_, Key_, Value_>, REST_...> {
  ...
  static_assert(CompileTimeMap<KeyType_, ValueType_, REST_...>::uniqueKey(Key_), "[CompileTimeMap] Keys are not unique!");
 ...
 public:
  ...
  template <ValueType_ value_>
  using findAll =
      cts_insert_front_if_t<KeyType_, typename CompileTimeMap<KeyType_, ValueType_, REST_...>::template findAllRecursive<value_>, Key_,
                            value_ == Value_>;
  ...
  */
  template <template <KeyType_, ValueType_> class F, class... Args>
  static constexpr bool forEach(Args&&... args) {
    return F<Key_, Value_>()(args...) && CompileTimeMap<KeyType_, ValueType_, REST_...>::template forEach<F, Args...>(args...);
  }
                ...
                ? F<Key_, Value_>()(args...) &&
                      ((begin == end) ? true
                                      : CompileTimeMap<KeyType_, ValueType_, REST_...>::template forEachRangePostBegin<F, Args...>(
                                            begin, end, args...))
                : CompileTimeMap<KeyType_, ValueType_, REST_...>::template forEachRangePreBegin<F, Args...>(begin, end, args...));
  }
  ...
  */
  static constexpr bool forEachRange(KeyType_ begin, KeyType_ end, std::function<bool(const KeyType_&, const ValueType_&)> f) {
    return containsRange(begin, end) &&
           (Key_ == begin
                ? f(Key_, Value_) &&
                      ((begin == end) ? true : CompileTimeMap<KeyType_, ValueType_, REST_...>::forEachRangePostBegin(begin, end, f))
  ...
  */
  ...
  static constexpr bool forEachRangePostBegin(KeyType_ begin, KeyType_ end, Args&&... args) {
    return Key_ == end
               ? F<Key_, Value_>()(args...)
               : F<Key_, Value_>()(args...) &&
                     CompileTimeMap<KeyType_, ValueType_, REST_...>::template forEachRangePostBegin<F, Args...>(begin, end, args...);
  }
  ...
  */
  static constexpr bool forEachRangePostBegin(KeyType_ begin, KeyType_ end, std::function<bool(const KeyType_&, const ValueType_&)> f) {
    return Key_ == end ? f(Key_, Value_)
                       : f(Key_, Value_) && CompileTimeMap<KeyType_, ValueType_, REST_...>::forEachRangePostBegin(begin, end, f);
  }
...
struct ctm_erase<KeyType_, ValueType_, CompileTimeMap<KeyType_, ValueType_, KeyValuePair<KeyType_, ValueType_, key, value>, KV_...>,
                 erase_key> {
  using type = typename std::conditional<
      key == erase_key, CompileTimeMap<KeyType_, ValueType_, KV_...>,
      ctm_insert_front_t<KeyType_, ValueType_,
...
struct ctm_add_to_values<KeyType_, ValueType_,
                         CompileTimeMap<KeyType_, ValueType_, KeyValuePair<KeyType_, ValueType_, Key_, Value_>, Kv_...>, summand> {
  using type =
      ctm_insert_front_t<KeyType_, ValueType_,
...
struct ctm_from_sets<KeyType_, ValueType_, CompileTimeSet<KeyType_, key, keys...>, CompileTimeSet<ValueType_, value, values...>> {
  ...
  using type = ctm_insert_front_t<
      KeyType_, ValueType_,
      typename ctm_from_sets<KeyType_, ValueType_, CompileTimeSet<KeyType_, keys...>, CompileTimeSet<ValueType_, values...>>::type,
      KeyValuePair<KeyType_, ValueType_, key, value>>;
};
...
template <typename KeyType_, typename ValueType_, typename TransformedKeyType_, typename... Keys_, typename TransformedValueType_,
          typename... Values_>
...
template <typename KeyType_, typename ValueType_, KeyType_ Key_, ValueType_ Value_, typename... KV_, typename TransformedKeyType_,
          typename... Keys_, typename TransformedValueType_, typename... Values_>
struct ctm_transform<TransformedKeyType_, TransformedValueType_,
  ...
  using type = ctm_insert_front_t<
      ...
      KeyValuePair<TransformedKeyType_, TransformedValueType_, CompileTimeMap<KeyType_, TransformedKeyType_, Keys_...>::at(Key_),
...
struct ctm_transform<TransformedKeyType_, ValueType_,
  ...
  using type = ctm_insert_front_t<
      ...
      KeyValuePair<TransformedKeyType_, ValueType_, CompileTimeMap<KeyType_, TransformedKeyType_, Keys_...>::at(Key_), Value_>>;
};
...
template <typename KeyType_, typename ValueType_, KeyType_ Key_, ValueType_ Value_, typename... KV_, typename TransformedValueType_,
          typename... Values_>
struct ctm_transform<KeyType_, TransformedValueType_,
                     CompileTimeMap<KeyType_, ValueType_, KeyValuePair<KeyType_, ValueType_, Key_, Value_>, KV_...>, void,
                     CompileTimeMap<ValueType_, TransformedValueType_, Values_...>> {
  using type = ctm_insert_front_t<
      ...
      typename ctm_transform<KeyType_, TransformedValueType_, CompileTimeMap<KeyType_, ValueType_, KV_...>, void,
                             CompileTimeMap<ValueType_, TransformedValueType_, Values_...>>::type,
      KeyValuePair<KeyType_, TransformedValueType_, Key_, CompileTimeMap<ValueType_, TransformedValueType_, Values_...>::at(Value_)>>;
};
 ...
 */
template <typename KeyType_, typename ValueType_, typename Map1_, typename Map2_, typename Map1Keys_, typename Map2Keys_,
          typename Map1Values_ = void, typename Map2Values_ = void>
struct ctm_combine {
  //! IDE helper type
  using type = CompileTimeMap<KeyType_, ValueType_>;
  //! Improve compiler output
...
template <typename KeyType_, typename ValueType_, typename KeyType1_, typename ValueType1_, typename KeyType2_, typename ValueType2_,
          typename... KV1_, typename... KVKeys1_, typename... KVValues1_, typename... KV2_, typename... KVKeys2_, typename... KVValues2_>
struct ctm_combine<KeyType_, ValueType_, CompileTimeMap<KeyType1_, ValueType1_, KV1_...>, CompileTimeMap<KeyType2_, ValueType2_, KV2_...>,
                   CompileTimeMap<KeyType1_, KeyType_, KVKeys1_...>, CompileTimeMap<KeyType2_, KeyType_, KVKeys2_...>,
  ...
  using type = std_utils::ctm_concatenate_t<
      ...
      ctm_transform_t<KeyType_, ValueType_, CompileTimeMap<KeyType1_, ValueType1_, KV1_...>,
                      CompileTimeMap<KeyType1_, KeyType_, KVKeys1_...>, CompileTimeMap<ValueType1_, ValueType_, KVValues1_...>>,
      ctm_transform_t<KeyType_, ValueType_, CompileTimeMap<KeyType2_, ValueType2_, KV2_...>,
                      CompileTimeMap<KeyType2_, KeyType_, KVKeys2_...>, CompileTimeMap<ValueType2_, ValueType_, KVValues2_...>>>;
};

//! @brief Template specialization for proper KeyMaps and empty ValueMaps
template <typename KeyType_, typename ValueType_, typename KeyType1_, typename KeyType2_, typename... KV1_, typename... KVKeys1_,
          typename... KV2_, typename... KVKeys2_>
struct ctm_combine<KeyType_, ValueType_, CompileTimeMap<KeyType1_, ValueType_, KV1_...>, CompileTimeMap<KeyType2_, ValueType_, KV2_...>,
                   CompileTimeMap<KeyType1_, KeyType_, KVKeys1_...>, CompileTimeMap<KeyType2_, KeyType_, KVKeys2_...>, void, void> {
  using type = std_utils::ctm_concatenate_t<KeyType_, ValueType_,
...
struct ctm_from_subenum {
  using ctsValues =
      cts_from_sequence_t<Enum_, make_index_sequence_range_t<Offset_, Offset_ - Skip_ + static_cast<std::size_t>(SubEnum_::SIZE)>>;
...
//! @brief Template specialization for CompileTimeMap with proper KeyType_ and ValueType_
template <typename InvertedKeyType_, typename InvertedValueType_, typename... Kv_, InvertedKeyType_ InvertedKey_,
          InvertedValueType_ InvertedValue_>
struct ctm_invert<InvertedKeyType_, InvertedValueType_,
                  CompileTimeMap<InvertedValueType_, InvertedKeyType_,
                                 KeyValuePair<InvertedValueType_, InvertedKeyType_, InvertedValue_, InvertedKey_>, Kv_...>> {
  static_assert(CompileTimeMap<InvertedValueType_, InvertedKeyType_,
                               KeyValuePair<InvertedValueType_, InvertedKeyType_, InvertedValue_, InvertedKey_>, Kv_...>::allValuesUnique(),
