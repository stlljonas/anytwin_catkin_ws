...
class enum_iterator final : public std::iterator<std::forward_iterator_tag, EnumType_> {  // NOLINT(readability-identifier-naming)
 public:
  ...
  constexpr explicit enum_iterator(const EnumType_ index) noexcept : id_{static_cast<integer_type>(index)} {}
  ...
  EnumType_ operator*() const { return static_cast<EnumType_>(id_); }
