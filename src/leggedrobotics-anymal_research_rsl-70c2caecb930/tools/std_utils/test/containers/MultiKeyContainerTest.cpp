...
enum class TestEnum : unsigned int { E_0 = 10, E_1 = 20, E_2 = 30 };

struct TestItem {
  explicit TestItem(double value) : value_(value) {}
  virtual ~TestItem() = default;
  double value_;
};

TEST(MultiKeyContainer, container) {  // NOLINT
  ...
  EXPECT_EQ(0u, container.getId("zero"));
  EXPECT_EQ(1u, container.getId("one"));
  EXPECT_EQ(2u, container.getId("two"));
