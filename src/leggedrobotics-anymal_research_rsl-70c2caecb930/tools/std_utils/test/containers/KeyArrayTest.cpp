...
TEST(KeyArray, insert) {  // NOLINT
  std_utils::KeyArray<TestEnum> keyContainer = {
      {std_utils::make_key(TestEnum::E_0, "E_0", 5),
  ...
  EXPECT_EQ(1u, keyContainer.at(TestEnum::E_1).getId());
  EXPECT_EQ(strcmp("E_1", keyContainer.at(TestEnum::E_1).getName()), 0);

  EXPECT_EQ(12u, keyContainer.get<TestEnum::E_2>().getId());
