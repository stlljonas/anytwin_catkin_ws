...
TEST(SmartPointerIterator, iterableClass) {  // NOLINT
...
TEST(SmartPointerIterator, dictionaryClass) {  // NOLINT
  MyUniquePtrDictionary<> unique_iterable;

  EXPECT_EQ(unique_iterable.size(), 5u);
...
TEST(SmartPointerIterator, observerPointer) {  // NOLINT
...

  {
    ...
    base_observer_container.insert(base_observer_container.end(), observer_iterable_a.s_begin(), observer_iterable_a.s_end());

    base_observer_container.insert(base_observer_container.end(), observer_iterable_b.s_begin(), observer_iterable_b.s_end());
