...
TEST(CompileTimeSet, forEachFunctor) {  // NOLINT
...
TEST(CompileTimeSet, forEachStdFunction) {  // NOLINT
  int counter = 0;
...
TEST(CompileTimeSet, forEachRangeFunctor) {  // NOLINT
...
TEST(CompileTimeSet, forEachRangeStdFunction) {  // NOLINT
  constexpr int increment = 2;
  int counter = 0;
  auto f = [&counter, increment](const EnumA& enumA) {
    counter += increment;
    return enumA != EnumA::A_1;
  };
...
TEST(CompileTimeSet, contains) {  // NOLINT
...
TEST(CompileTimeSet, containsRange) {  // NOLINT
...
TEST(CompileTimeSet, size) {  // NOLINT
  EXPECT_EQ(5u, setA::size());
  EXPECT_EQ(3u, subSetA1::size());
  EXPECT_EQ(2u, subSetA2::size());
}

TEST(CompileTimeSet, concatenate) {  // NOLINT
...
TEST(CompileTimeSet, insert_back) {  // NOLINT
...
TEST(CompileTimeSet, insert_front) {  // NOLINT
...
TEST(CompileTimeSet, insert_front_if) {  // NOLINT
...
TEST(CompileTimeSet, erase) {  // NOLINT
...
TEST(CompileTimeSet, from_sequence) {  // NOLINT
  using setASequence = std_utils::cts_from_sequence_t<EnumA, std_utils::make_index_sequence_t<static_cast<int>(EnumA::SIZE)>>;

  constexpr bool setAFromDequenceSuccessful = std::is_same<setASequence, setA>::value;
  EXPECT_TRUE(setAFromDequenceSuccessful);
}

TEST(CompileTimeSet, from_enum) {  // NOLINT
