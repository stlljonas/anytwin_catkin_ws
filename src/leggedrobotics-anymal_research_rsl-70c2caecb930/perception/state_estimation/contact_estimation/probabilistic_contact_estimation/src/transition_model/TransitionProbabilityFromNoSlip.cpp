/*
 * TransitionProbabilityFromNoSlip.cpp
 *
...
TransitionProbabilityFromNoSlip::TransitionProbabilityFromNoSlip(AD::ContactEnum contactEnum, unsigned int numDofLimb) :
...
bool TransitionProbabilityFromNoSlip::initialize(double dt) {
...
bool TransitionProbabilityFromNoSlip::loadParameters(const TiXmlHandle& handle) {
...
bool TransitionProbabilityFromNoSlip::addVariablesToLog(const std::string& ns) const {
  const auto keyName = std::string(AD::mapKeyEnumToKeyName<AD::ContactEnum, AD::BranchEnum>(contactEnum_));
  signal_logger::add(prob_(noTransitionId), "/pTransitionNoSlipToNoSlip_" + keyName, "/contact_estimation", "-");
  signal_logger::add(prob_(transitionId), "/pTransitionNoSlipToSlip_" + keyName, "/contact_estimation", "-");
  return true;
}

bool TransitionProbabilityFromNoSlip::advance(double dt, const State* const state) {
  ...
  // Clip to box constraints and normalize.
