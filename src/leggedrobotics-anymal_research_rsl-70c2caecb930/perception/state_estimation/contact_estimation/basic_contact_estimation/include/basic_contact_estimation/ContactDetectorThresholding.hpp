...
class ContactDetectorThresholding : public ContactDetectorBase {
 public:
  enum class ThresholdingMethod : unsigned int { NORM = 0, ZONLY, ZCLAMPED, ALL, ALLCLAMPED, XONLY, XYONLY };
  class Config {
   public:
    Config();
    ~Config();
 ...
 public:
  explicit ContactDetectorThresholding(const std::string& name, const Config& config = Config());
  ~ContactDetectorThresholding() override;
