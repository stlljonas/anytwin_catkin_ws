...
ContactDetectorThresholding::Config::Config() = default;

ContactDetectorThresholding::Config::~Config() = default;

ContactDetectorThresholding::ContactDetectorThresholding(const std::string& name, const Config& config)
    : ContactDetectorBase(name), config_(config) {}

ContactDetectorThresholding::~ContactDetectorThresholding() = default;
...
ContactDetectorBase::ContactState ContactDetectorThresholding::getStateFromForce() {
  ...
  switch (config_.forceThresholdingMethod_) {
    ...
    case ThresholdingMethod::XYONLY:
      if (std::abs(wrench_.getForce().x() * wrench_.getForce().x() + wrench_.getForce().y() * wrench_.getForce().y()) >
    ...
    case ThresholdingMethod::ZCLAMPED:
      if (wrench_.getForce().z() > config_.thresholdForce_.z() && wrench_.getForce().z() < config_.upperThresholdForce_.z()) {
        return ContactState::CLOSED;
      } else {
        return ContactState::OPEN;
      }
