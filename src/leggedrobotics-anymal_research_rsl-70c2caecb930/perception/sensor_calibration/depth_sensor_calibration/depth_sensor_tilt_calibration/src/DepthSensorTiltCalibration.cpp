/*
 * DepthSensorTiltCalibration.cpp
 *
...
namespace depth_sensor_tilt_calibration {

DepthSensorTiltCalibration::DepthSensorTiltCalibration(ros::NodeHandle& nodeHandle)
    : nodeHandle_(nodeHandle),
      pointCloud_(new PointCloud)
{
  ROS_INFO("Depth sensor tilt calibration node started.");
  readParameters();
  pointCloudSubscriber_ = nodeHandle_.subscribe(pointCloudTopic_, 1, &DepthSensorTiltCalibration::addPointCloud, this);
  pointCloudPublisher_ = nodeHandle_.advertise<sensor_msgs::PointCloud2>("point_cloud_inliers", 1, true);
}

DepthSensorTiltCalibration::~DepthSensorTiltCalibration() {}

bool DepthSensorTiltCalibration::readParameters()
{
  ...
  nodeHandle_.param("camera_name", cameraName_, string("camera_name"));
...
void DepthSensorTiltCalibration::addPointCloud(const PointCloud& pointCloud)
{
...
bool DepthSensorTiltCalibration::calibrate()
{
...
bool DepthSensorTiltCalibration::computeRotation(const Eigen::Vector3d& surfaceNormal, const std::string& frameId)
{
  ...
  cout << "======Copy-friendly Output=====" << endl;
  cout << cameraName_ << "_pitch: " << eulerVector(1) << endl;
  cout << cameraName_ << "_roll: " << eulerVector(0) << endl;
  cout << "===============================" << endl;

  return true;
}

void DepthSensorTiltCalibration::publishInliers(pcl::PointIndices::ConstPtr inlierIndices)
{
...
void DepthSensorTiltCalibration::exit()
{
